---
name: 大数据处理
description: "通用数据分析技能：当需要基于 `db_query`/`db_export` 系列数据库工具或本地 Excel/CSV 文件做结构探测、聚合分析、趋势判断、导出落地与专业图表方案设计时使用；适用于字段探索、全景分析、专题下钻、Excel 产物生成与中文字段检索验证。"
---

# 核心目标
- 产出可复核的分析结论，而不是只给“看起来合理”的描述。
- 先分析再建议，不擅自修改数据库或原始文件。
- 仅基于当前可见资源工作：`db_query`/`db_export` 系列工具、或本地表格文件。
...
- 产物型任务默认流程：先用 `db_query*` 做计数/样本校验，再用不带 `LIMIT/OFFSET` 的最终明细 SQL 调 `db_export*` 直接生成 `xlsx/csv`。
...
- 样本查询、聚合查询或字段不完整的 SQL 只能用于验证，不能作为正式导出全量明细的数据源。
- 正式导出前，必须自检这三点：
  1. 导出 SQL 与最终筛选口径一致；
  2. 导出 SQL 不带 `LIMIT/OFFSET`；
  3. 导出字段就是用户需要落盘的最终字段集。
- 如果你先做了样本查询，再要导出全量数据，应该重新构造一条不带 `LIMIT/OFFSET` 的明细 SQL。
...
3. 为正式导出构造“不带 `LIMIT/OFFSET` 的最终明细 SQL”。
