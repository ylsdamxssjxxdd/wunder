...
**单用例评分（满分 100）**：
- 流程合规 20 分：必须出现 `技能调用` → 复制脚本 → `读取文件` → 替换 `OUTPUT_FILE` 与 `SLIDES` → `node build.js` → `列出文件`。
- 产物正确 15 分：`列出文件` 中存在目标文件名；不得只生成 `output.pptx`。
- 内容匹配 25 分：`SLIDES` 块替换命中且与输入标题/要点一致；`OUTPUT_FILE` 替换命中。
- 视觉与样式 30 分（更强调观感，不再只看文本）：
