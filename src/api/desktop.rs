...
pub fn router() -> Router<Arc<AppState>> {
    Router::new()
        ...
        .route(
            "/wunder/desktop/reset_work_state",
            post(desktop_reset_work_state),
        )
        .route(
...
#[derive(Debug, Clone, Serialize)]
struct DesktopResetWorkStateSession {
    agent_id: String,
    session_id: String,
}

#[derive(Debug, Clone, Serialize)]
struct DesktopResetWorkStateSummary {
    cancelled_sessions: usize,
    cancelled_tasks: usize,
    cancelled_team_runs: usize,
    cleared_workspaces: usize,
    removed_workspace_entries: u64,
    fresh_main_sessions: Vec<DesktopResetWorkStateSession>,
}
...
async fn desktop_reset_work_state(
    ...
    let (team_runs, _) = state
        .user_store
        .list_team_runs(&user_id, None, None, 0, 4096)
        .map_err(|err| internal_error(err.to_string()))?;
    ...
    for variant in state.workspace.scoped_user_id_variants(&user_id, None) {
    ...
    for agent in &agents {
        ...
        for variant in state
            .workspace
            .scoped_user_id_variants(&user_id, Some(&agent.agent_id))
        {
            workspace_scopes.insert(variant);
        }
    }
    ...
    for scope in &workspace_scopes {
        removed_workspace_entries = removed_workspace_entries.saturating_add(
            state
                .workspace
                .clear_workspace_contents(scope)
                .map_err(|err| internal_error(err.to_string()))?,
        );
    }
    ...
    let default_session_id = state
        .kernel
        .thread_runtime
        .create_fresh_main_session_id(&user_id, "", "desktop_reset_work_state")
        .await
        .map_err(|err| internal_error(err.to_string()))?;
    fresh_main_sessions.push(DesktopResetWorkStateSession {
        agent_id: String::new(),
    ...
    for agent in &agents {
        let session_id = state
            .kernel
            .thread_runtime
            .create_fresh_main_session_id(&user_id, &agent.agent_id, "desktop_reset_work_state")
            .await
            .map_err(|err| internal_error(err.to_string()))?;
        fresh_main_sessions.push(DesktopResetWorkStateSession {
    ...
    Ok(Json(json!({
        "data": DesktopResetWorkStateSummary {
            cancelled_sessions,
            cancelled_tasks,
            cancelled_team_runs,
