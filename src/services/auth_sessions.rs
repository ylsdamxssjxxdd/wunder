...
impl AuthSessionService {
    ...
    pub fn register(&self, user_id: &str, connection_id: &str, sender: mpsc::Sender<Message>) {
        let normalized_user_id = user_id.trim();
        let normalized_connection_id = connection_id.trim();
        if normalized_user_id.is_empty() || normalized_connection_id.is_empty() {
            return;
        }
    ...
    pub async fn force_logout_user(&self, user_id: &str) {
        let normalized_user_id = user_id.trim();
        ...
        let targets = self
            ...
            .filter_map(|entry| {
                ...
                if connection.user_id != normalized_user_id || connection_id.is_empty() {
                    return None;
                }
        ...
        for (connection_id, sender) in targets {
            ...
            tokio::spawn(async move {
                let _ = sender
                    .send(Message::Text(forced_logout_error.into()))
                    .await;
...
mod tests {
    ...
    async fn force_logout_user_pushes_error_and_close() {
        ...
        service.register("alice", "ws_1", tx);

        service.force_logout_user("alice").await;
