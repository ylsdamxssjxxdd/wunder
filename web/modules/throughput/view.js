    ...
    <div class="tp-layout">
      <aside class="tp-config monitor-block">
        ...
        <form id="tpForm">
          ...
          <div class="form-row"><label for="tpInput">${l("input")}</label><select id="tpInput">${options([1024,2048,8192,16384,32768,65536,131072,262144,524288,1048576])}</select><p class="muted">${l("inputHint")}</p></div>
          <div class="form-row"><label for="tpOutput">${l("output")}</label><select id="tpOutput">${options([1024,2048,4096,8192])}</select><p class="muted">${l("outputHint")}</p></div>
      ...
      <div class="tp-results">
        ...
        <section class="monitor-block tp-history">
          ...
          <div class="tp-table-wrap"><table class="monitor-table"><thead><tr>
            ${["select","time","model","input","target","actualOutput","decode","ttft","status","view"].map((key) => `<th scope="col">${l(key)}</th>`).join("")}
...
export function details(element, item) {
  ...
  const fields = [["input", tokenLabel(item.config.input_tokens)], ["target", number(item.config.output_tokens,0)],
...
export function history(element, items, selected, viewed) {
  element.innerHTML = [...items].reverse().map((item) => `<tr class="${viewed === item.id ? "tp-viewed" : ""}">
    ...
    <td>${escape(formatTimestamp(item.started_at))}</td><td>${escape(item.config.model_name)}</td><td>${tokenLabel(item.config.input_tokens)}</td><td>${tokenLabel(item.config.output_tokens)}</td>
  ...
  </tr>`).join("") || `<tr><td colspan="10" class="tp-empty">${l("none")}</td></tr>`;
}
