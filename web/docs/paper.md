...
## 摘要
随着大模型逐步成为通用智能底座，面向复杂任务的“智能体 + 工具编排”正成为生产系统的主流形态。但在真实场景中，模型能力、工具协议、并发访问、上下文膨胀、安全隔离与可观测性之间存在显著矛盾。本文以 Wunder 智能体调度平台为对象，系统性阐述其设计与实现过程。平台基于 Rust/Axum 构建统一核心能力，并形成 `wunder-server`（多租户服务形态）、`wunder-cli`（本地命令行形态）与 `wunder-desktop`（本地图形界面形态）三种运行方式；在接口层提供 `/wunder` 统一入口，采用 WebSocket 优先、SSE 兜底的流式交互机制，并统一编排 MCP、Skills、知识库、自建/共享工具。系统通过 Orchestrator 实现多轮工具调用与任务调度，通过工作区与持久化分层实现 server 与本地形态的数据边界，通过上下文压缩、长期记忆、监控与性能评估构建工程化闭环。本文从需求分析、总体架构、核心流程、关键算法、数据与安全、性能评估与运维实践等方面进行分析，聚焦“同一调度内核跨运行形态复用”的实现机理，为同类智能体平台的工程落地提供可复用思路。

## 关键词
智能体调度；Skills；工具编排；多模型适配；MCP；WebSocket/SSE；上下文压缩；多用户工作区；多形态运行；CLI/Desktop；额度治理；A2A
...
## 3 需求分析与设计目标
### 3.1 功能需求
平台的功能需求不是简单的功能清单，而是对“如何协同模型与工具”的工作方式进行约束。统一入口与流式交互是第一层要求：/wunder 承担稳定的请求面，使前端与外部系统不必理解内部多模型与多工具的差异；平台以 WebSocket 作为主流式通道、SSE 作为兜底通道，把“思考—行动—反馈”的中间过程可视化，让用户在长任务中持续获得进度感，同时为监控与回放提供统一事件切片。
...
第五层要求是跨运行形态一致性。平台不仅服务于多租户部署，也要支持开发者本地调试与终端用户桌面使用。因此 `wunder-server`、`wunder-cli` 与 `wunder-desktop` 必须共享同一调度内核、工具协议与提示词体系，同时在持久化路径、鉴权模式与资源开销上保持可区分策略：server 面向组织治理，CLI 面向单用户轻量执行，Desktop 面向可视化本地交互。
...
对于本地形态，平台采用“同内核、轻部署”策略：CLI 直接以当前启动目录作为工作目录并在 `WUNDER_TEMP/` 持久化运行状态；Desktop 以 Tauri 桌面窗口承载用户界面，内置本地 bridge 服务并将状态落盘到 `WUNDER_TEMPD/`，默认工作目录为 `WUNDER_WORK/`。这种分层让同一调度能力既能服务云端治理，也能服务本地快速使用。

![部署拓扑](./diagrams/system-intro/07-deployment-topology.svg)

### 4.2 模块职责划分（概念层）
模块划分强调边界清晰与职责收敛。API 层聚焦“入口统一与协议稳定”，将鉴权、限流与流式输出封装在统一路由中；AgentRuntime 负责主线程映射、排队调度与会话分叉；Orchestrator 聚焦会话生命周期与多轮推理—工具执行闭环，是系统的“决策中枢”。ChannelHub 与 GatewayHub 分别承担渠道接入与控制平面连接治理，避免业务链路与通信链路耦合。工具执行层统一封装内置工具、MCP/A2A、Skills、知识库与用户工具，以减少模型感知差异。Prompt Builder 作为“认知接口”将工具规格、技能说明与工作区摘要整合为可被模型理解的上下文，并通过缓存降低重复构建成本。监控与存储层保证事件、日志、记忆与评估结果可追踪可回放；工作区层提供文件与资产隔离，使多用户环境下的数据互不污染。
...
| `wunder-desktop` | 本地普通用户 | Tauri 窗口 + 本地 bridge，WebSocket 优先 | 程序同级 `WUNDER_TEMPD/` + `WUNDER_WORK/` | GUI 体验、本地优先、可选远端接入与自动回退 |
...
在本地形态上，CLI 与 Desktop 进一步扩展了使用面：CLI 提供 codex 风格 TUI 与脚本化入口，适合开发与自动化；Desktop 提供与用户前端同构的 GUI 体验，适合普通用户与离线/弱网场景。两者共享同一调度与工具内核，使“本地试验—服务端上线”成本显著降低。
...
- Desktop 形态采用 Tauri 桌面封装 + 本地 bridge；可在本地模式运行，也可按 `remote_gateway` 切远端并失败自动回退。
...
## 15 结论与展望
本文围绕 Wunder 智能体调度平台，从架构设计到关键实现进行了系统论述。平台的核心贡献不仅在于统一入口、工具编排、上下文压缩、长期记忆与监控评估形成的工程闭环，也在于验证了“同一调度内核跨运行形态复用”的可行性：`wunder-server` 面向多租户治理，`wunder-cli` 面向本地轻量调试，`wunder-desktop` 面向本地可视化交互，三者在协议与能力层保持一致，在部署与治理层按场景分化。该设计使系统能够同时满足多用户、多工具、多模型的复杂需求，并具备良好的可维护性与可扩展性。未来工作可进一步增强知识库检索质量、完善技能执行与知识库持久化、引入更细粒度的权限控制与审计机制，并探索面向多集群的弹性调度与成本优化策略。
