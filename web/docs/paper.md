...
### 3.2 非功能需求
非功能需求决定平台能否长期稳定运行。并发与可用性方面，平台需要在高峰负载下保持可预测行为，因此采用“同一 session_id 互斥 + 全局并发上限”的双重约束，并在主会话繁忙时支持排队或自动分叉新会话；问询面板进入 waiting 不视为繁忙，用户选择可直接继续处理。同时确保 SSE 断开不等同于任务失败，降低网络波动对完成率的影响。
...
![部署拓扑](./diagrams/system-intro/07-deployment-topology.svg?v=20260130-01)
