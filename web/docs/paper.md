...
在多智能体场景中，单次会话内的临时角色编排难以复用。为解决“能力沉淀与跨环境迁移”问题，Wunder 在调度层之上引入蜂巢协议（Hive Packaging Protocol, HPP），将协作能力封装为可导入/导出的标准资产。协议采用三层抽象：`HivePack`（蜂群组织层）、`WorkerPack`（角色执行层）、`SkillPack`（能力沉淀层），将“组织结构、角色职责、技能依赖”纳入统一清单治理。
