...
## 关键词
智能体调度；工具编排；多模型适配；MCP；SSE；上下文压缩；多用户工作区；额度治理；A2A
...
从研究趋势看，开放式智能体逐渐依赖“可增长的技能库”和“标准化流程”。Voyager 展示了持续扩展技能库对长期探索的作用[21]，MetaGPT 以 SOP 驱动多角色协作以降低错误传播[22]。这进一步说明技能与流程是可复用的过程资产，而非一次性工具调用。Wunder 将 Skills 作为共享的流程片段管理，并配合工作区与权限治理，使这些资产能够被长期沉淀与跨团队复用。
...
事件流还包含 `quota_usage` 事件，记录每次模型调用的额度消耗与剩余额度，便于前端提示与审计追踪。
...
在额度治理方面，管理端可调整注册用户的每日额度与权限，用户端在回复后展示本次消耗与剩余额度，形成清晰的成本反馈闭环。
