...
## 摘要
随着大模型逐步成为通用智能底座，面向复杂任务的“智能体 + 工具编排”正成为生产系统的主流形态。但在真实场景中，模型能力、工具协议、并发访问、上下文膨胀、安全隔离与可观测性之间存在显著矛盾。本文以 Wunder 智能体调度平台为对象，系统性阐述其设计与实现过程。平台基于 Rust/Axum 构建统一核心能力，并形成 `wunder-server`（多租户服务形态）、`wunder-cli`（本地命令行形态）与 `wunder-desktop`（本地图形界面形态）三种运行方式；在产品层已经从“单会话问答”演进为“通信/办公化智能体工作台”，支持用户体系聊天（用户↔智能体、用户↔用户）并重构统一 Messenger 前端壳。系统通过 `/wunder` 统一入口（WebSocket 优先、SSE 兜底）编排 MCP、Skills、知识库与用户工具，围绕 **形态协同 / 租户治理 / 智能体协作 / 工具生态 / 接口开放** 组织能力。当前阶段以 desktop 为主交付形态，并通过本地优先 + 远端网关接入的端云协同路径推进上线部署。本文从需求分析、总体架构、核心流程、关键算法、数据与安全、性能评估与运维实践等方面进行分析，聚焦“同一调度内核跨运行形态复用”的实现机理，为同类智能体平台的工程落地提供可复用思路。

## 关键词
智能体调度；通信/办公化智能体工作台；形态协同/租户治理/智能体协作/工具生态/接口开放；用户体系聊天；MCP；WebSocket/SSE；上下文压缩；Desktop-first；端云协同；额度治理；A2A
...
Wunder 的设计目标是构建一个“可通信、可协作、可治理”的智能体调度平台：对开发者而言一切是接口（API/配置/工具），对模型而言一切皆工具（可调用、可组合、可治理）。平台能力以 **形态协同 / 租户治理 / 智能体协作 / 工具生态 / 接口开放** 为核心框架，在工程上强调 desktop-first 交付与端云协同部署，在系统上兼顾多模型治理、并发隔离、上下文压缩、监控评估与安全策略。本文将展示 Wunder 在这些维度的系统设计思路与具体实现。
...
## 3 需求分析与设计目标
### 3.1 功能需求
平台的功能需求不是简单的功能清单，而是对“如何协同模型、工具与通信场景”的工作方式进行约束。当前产品目标已经从问答界面升级为通信/办公化工作台，因此需求设计以“五维能力框架”为总结构：形态协同承载不同场景，租户治理保障组织治理，智能体协作支撑应用化协作，工具生态保证执行能力，接口开放保证接入与互操作。统一入口与流式交互是第一层要求：/wunder 承担稳定请求面，使前端与外部系统不必理解内部多模型与多工具差异；平台以 WebSocket 作为主流式通道、SSE 作为兜底通道，把“思考—行动—反馈”的中间过程可视化，让用户在长任务中持续获得进度感，同时为监控与回放提供统一事件切片。
...
第五层要求是跨运行形态一致性与角色分工。平台不仅服务于多租户部署，也要支持开发者本地调试与终端用户桌面使用。因此 `wunder-server`、`wunder-cli` 与 `wunder-desktop` 必须共享同一调度内核、工具协议与提示词体系，同时在持久化路径、鉴权模式与资源开销上保持可区分策略：server 面向组织治理，CLI 面向单用户轻量执行，Desktop 作为当前主交付形态承载通信化 GUI，并通过 remote gateway 支持端云协同。

### 3.2 非功能需求
非功能需求决定平台能否长期稳定运行。并发与可用性方面，平台需要在高峰负载下保持可预测行为，因此采用“同一 session_id 互斥 + 全局并发上限”的双重约束，并在主会话繁忙时支持排队或自动分叉新会话；问询面板进入 waiting 不视为繁忙，用户选择可直接继续处理。同时保证 WebSocket 断开可退化为 SSE，且可基于 stream_events 续传回放，降低网络波动对完成率的影响。
...
#### 4.1.1 部署拓扑
部署视角下，系统按“访问方—运行服务—外部依赖—挂载目录”分层组织。对于 server 形态，核心服务集中于容器内运行，外部依赖与共享挂载目录保持清晰边界，便于部署与运维治理。向量数据库（Weaviate）与主数据库并列作为运行服务的一部分部署，以降低索引与检索的网络开销，并便于统一的资源与容错治理。

对于本地形态，平台采用“同内核、轻部署”策略：CLI 直接以当前启动目录作为工作目录并在 `WUNDER_TEMP/` 持久化运行状态；Desktop 以桌面窗口承载用户界面，内置本地 bridge 服务并将状态落盘到 `WUNDER_TEMPD/`，默认工作目录为 `WUNDER_WORK/`。本地 bridge 默认监听 `127.0.0.1:18123`，端口占用时自动回退随机空闲端口；前端通过 `GET /config.json` 与 `GET /wunder/desktop/bootstrap` 自举运行参数。这种分层让同一调度能力既能服务云端治理，也能服务本地快速使用。

![部署拓扑](./diagrams/system-intro/07-deployment-topology.svg)

### 4.2 模块职责划分（概念层）
模块划分强调边界清晰与职责收敛。API 层聚焦“入口统一与协议稳定”，将鉴权、限流与流式输出封装在统一路由中，并把 `core/chat/user_world/user_agents/user_tools/desktop` 作为用户侧主能力面；AgentRuntime 负责主线程映射、排队调度与会话分叉；Orchestrator 聚焦会话生命周期与多轮推理—工具执行闭环，是系统的“决策中枢”。ChannelHub 与 GatewayHub 分别承担渠道接入与控制平面连接治理，避免业务链路与通信链路耦合。工具执行层统一封装内置工具、MCP/A2A、Skills、知识库与用户工具，以减少模型感知差异。Prompt Builder 作为“认知接口”将工具规格、技能说明与工作区摘要整合为可被模型理解的上下文，并通过缓存降低重复构建成本。监控与存储层保证事件、日志、记忆与评估结果可追踪可回放；工作区层提供文件与资产隔离，使多用户环境下的数据互不污染。
...
`wunder-cli` 与 `wunder-desktop` 复用同一核心模块，但采用本地优先初始化策略：CLI 使用 `cli_default` 轻量选项关闭不必要后台循环，并在配置缺失时自动生成 `WUNDER_TEMP/config/wunder.yaml`；Desktop 使用 `desktop_default` 轻量选项，默认关闭 channels/gateway/agent_queue，保留 cron 调度能力，并通过本地 bridge 暴露 `GET /config.json` 与 `GET /wunder/desktop/bootstrap` 供前端引导。Desktop 运行时会在本地模式自动注入本地 token 以降低门槛；若启用并连通 `remote_gateway`，请求会切向远端 `/wunder`，本地 token 自动退出用户登录态，改为远端正常鉴权流程；远端失败时再自动回退本地。

### 4.4 多形态运行架构（server/cli/desktop）
为保证“同一能力在不同场景可复用”，Wunder 将运行形态抽象为统一内核下的三个运行面，差异主要体现在入口协议、持久化路径与治理策略。

| 运行形态 | 主要用户 | 入口与交互 | 默认持久化 | 治理特征 |
| --- | --- | --- | --- | --- |
| `wunder-server` | 组织管理员与普通用户 | `/wunder`、WebSocket/SSE、管理端与用户端前端 | `workspaces/<user_id>` + PostgreSQL（默认） | 多租户、配额、审计、网关与渠道治理 |
| `wunder-cli` | 开发者/高级用户 | 终端命令、TUI、JSONL 事件输出 | 启动目录 `WUNDER_TEMP/`（SQLite + 配置 + 会话） | 单用户轻量运行、快速调试、tool_call/function_call 快速切换 |
| `wunder-desktop` | 本地普通用户（当前主交付） | 桌面窗口 + 本地 bridge，WebSocket 优先 | 程序同级 `WUNDER_TEMPD/` + `WUNDER_WORK/` | 通信化 GUI、本地优先、可选远端接入与自动回退 |
...
流式输出是该流程的关键组成。平台默认通过 WebSocket 传输中间步骤、工具调用与最终答案，在不可用时自动退化到 SSE，同一语义事件会持续写入 stream_events，使前端展示、日志分析与回放复用同一事实切片。
...
### 5.1.1 主链路概念与并发语义
为了避免概念混淆，Wunder 将主链路明确为一条可追踪、可回放的结构化路径：

- **用户（user_id）**：请求的归属与工作区边界。
- **智能体应用（agent_id）**：面向业务场景的应用级配置与主会话绑定。
- **会话/智能体线程（session_id）**：具体执行上下文，互斥锁以 session_id 为粒度，避免同一会话并行执行。
- **智能体循环（agent_loop）**：多轮模型—工具闭环，围绕 user_round/model_round 与状态机推进。
- **流式事件（WS/SSE）**：对每次模型输出、工具调用与最终回答的事实记录，既驱动前端展示，也支撑回放与审计。

并发语义与该链路一一对应：每个 `user_id + agent_id` 维护主会话，但当主会话忙且未显式指定 `session_id` 时会自动分叉新会话继续处理；当显式 `session_id` 忙时，依据 `agent_queue` 决定排队或返回 `USER_BUSY`。问询面板进入 `waiting` 表示“等待选择”而非“繁忙”，用户选择后直接进入下一轮 `agent_loop`，不会触发排队或繁忙错误。
...
- 可观测与可回放的事件流：WS/SSE 统一事件语义对“模型输出—工具调用—工具结果—最终回复”提供逐步切片，使前端展示、监控统计与故障追踪共享同一事实源。
...
这种“双协议统一治理”在多形态运行中尤为关键。CLI 可通过 `/tool-call-mode`（`/mode`）在会话内快速切换模式，Desktop 则在设置页通过请求级覆盖实现模式切换；两者都复用同一回填逻辑，保证本地与 server 行为一致。
...
## 7 调度引擎设计与实现
### 7.1 资源与并发控制
资源与并发控制决定平台在高负载场景中的稳定性。平台采用 session_id 维度互斥（`session_locks` + TTL 心跳），保证同一会话不会并行执行；工作区仍按 `user_id + agent_id` 隔离，因此同一用户可并行多个会话/智能体应用。

当 `agent_queue.enabled=true` 时，忙请求进入队列等待空闲；当未启用队列且显式 `session_id` 处于 running/cancelling 时返回 `USER_BUSY`。未传 `session_id` 且主会话忙时会自动分叉独立会话继续处理，避免阻塞主线程；问询面板进入 `waiting` 仅表示待选择，不视为繁忙。

并发控制之外，额度治理对注册用户的模型调用进行精细约束，与全局并发上限形成互补，确保成本与资源消耗可预期。

并发控制不仅是性能优化，也是公平与一致性的约束。同一 session_id 互斥避免同一上下文被并行修改导致的语义漂移；全局上限提供背压，保证系统在高负载下维持可预测的响应曲线。对长期任务而言，排队与自动分叉的组合还能减少“长任务占用资源导致短任务饥饿”的风险，使平台具备更稳定的服务质量。
...
### 7.3 WS/SSE 事件流与持久化
WS/SSE 事件流是平台可观测性的核心基础。平台将模型输出、工具调用、工具结果与最终回答统一为事件序列，优先经 WebSocket 推送，失败时由 SSE 承接，使前端与监控系统共享同一视图。当事件队列溢出或网络中断时，事件仍会落库并支持续传，从而避免“看不到过程”或“过程丢失”的问题。
...
### 7.6 蜂群协议（HPP）与多智能体资产化
在多智能体场景中，单次会话内的临时角色编排难以复用。为解决“能力沉淀与跨环境迁移”问题，Wunder 在调度层之上引入蜂巢协议（Hive Packaging Protocol, HPP），将协作能力封装为可导入/导出的标准资产。协议采用三层抽象：`HivePack`（蜂群组织层）、`WorkerCard`（工蜂身份层）、`SkillPack`（能力沉淀层），将“组织结构、角色职责、工具/技能依赖、预设问题”纳入统一清单治理；其中 `WorkerCard` 使用单文件描述单个智能体。

协议化后，多智能体协作不再依赖人工重复配置，而是通过内置蜂群工具形成稳定闭环：`discover`（识别与拆解）→ `dispatch`（并行分派）→ `wait`（等待与同步）→ `merge`（汇总与交付）。这一机制使复杂任务能够在角色并行执行与结果集中治理之间取得平衡，并为审计与回放提供稳定语义锚点。

```mermaid
flowchart LR
  U["任务目标"] --> D["discover: 子任务与角色映射"]
  D --> P["dispatch: 工蜂并行执行"]
  P --> W["wait: 屏障等待与重试"]
  W --> M["merge: 汇总/冲突消解/统一输出"]
```

为应对导入时的命名冲突与资产演进，协议定义两种冲突策略：`auto_rename_only`（默认）与 `update_replace`（替换更新）。前者确保本地资产零覆盖，后者支持版本升级时的原地替换。该双策略设计兼顾了“安全导入”与“快速升级”，提升了蜂群资产在组织内推广和持续迭代的可操作性。
...
在多形态运行下，工作区策略按场景分层：server 以 `workspaces/<user_id>` 提供多用户隔离；CLI 采用“启动目录即工作目录”，以减少开发路径切换；Desktop 默认使用 `WUNDER_WORK/` 并支持容器目录映射，兼顾普通用户可视化操作与可控边界。
...
当前实现已经形成覆盖会话调度、工具日志、渠道接入、网关控制平面、用户权限、评估与向量元数据的 37 张核心关系表，并通过 StorageBackend 抽象保持 PostgreSQL 与 SQLite 的语义一致。
...
- 超时与重试策略应对短暂故障，降低失败率。
- 事件回放确保 WS 中断可退化至 SSE，且全过程可续与可追踪。
...
- 资源有界与背压：同一 session_id 互斥 + 全局并发上限提供稳定吞吐基线，配合忙时排队/自动分叉降低阻塞；模型调用额度与 max_rounds 约束避免长链路占用过久；上下文预算与工具结果截断让每轮推理都有明确上界。
- 故障隔离与退化：工作区隔离与沙盒执行限制风险扩散，工具白名单与权限边界减少误用；外部工具具备超时与失败回退，WS 断线可退化到 SSE，执行不中断并可复用事件回放。
...
## 14 运维实践与可扩展性
### 14.1 管理与用户前端实践
管理端与用户端的分工体现“治理与使用分离”，但在当前阶段用户端已经重构为通信/办公化壳层：

- 管理端聚焦配置、监控与评估，使平台能力可被持续调整与审计。
- 用户端聚焦消息与协作，统一 Messenger 壳承载智能体会话、用户会话、群聊、工具与文件能力。
- 默认入口收敛为 `/app/home`（Desktop 为 `/desktop/home`），降低应用化导航成本。

在本地形态上，CLI 与 Desktop 进一步扩展了使用面：CLI 提供 codex 风格 TUI 与脚本化入口，适合开发与自动化；Desktop 作为当前主交付，提供通信软件化 GUI 与本地优先体验，并可平滑切入远端协同。两者共享同一调度与工具内核，使“本地试验—服务端上线”成本显著降低。
...
- server 形态通过容器化部署解耦主服务、数据库与沙盒服务，降低单点故障影响。
- CLI 形态采用本地单目录部署，依赖 `WUNDER_TEMP/` 自包含持久化，便于迁移与回滚。
- Desktop 形态采用桌面壳 + 本地 bridge；默认 `127.0.0.1:18123`，端口冲突自动回退随机端口；可在本地模式运行，也可按 `remote_gateway` 切远端并失败自动回退。
- 端云协同阶段建议采用“本地体验入口 + 云端统一治理”部署方式，保证弱网可用性与集中管控并存。
- 基础配置 + 覆盖配置支持不同环境快速切换，并允许管理端或本地设置动态调整。
...
## 15 结论与展望
本文围绕 Wunder 智能体调度平台，从架构设计到关键实现进行了系统论述。平台当前的核心贡献是把产品能力收敛到“五维能力框架”——**形态协同 / 租户治理 / 智能体协作 / 工具生态 / 接口开放**——并在同一调度内核下完成通信/办公化升级：用户体系聊天、统一 Messenger 壳、工具编排、上下文压缩、长期记忆与监控评估形成闭环。`wunder-server` 面向多租户治理，`wunder-cli` 面向本地轻量调试，`wunder-desktop` 作为主交付形态承载日常协作，三者在协议与能力层保持一致，在部署与治理层按场景分化。该设计使系统能够同时满足多用户、多工具、多模型的复杂需求，并为端云协同上线提供可落地路径。未来工作可进一步增强知识库检索质量、完善技能执行与知识库持久化、引入更细粒度的权限控制与审计机制，并探索面向多集群的弹性调度与成本优化策略。
