# 子 agent 落地方案（对齐 OpenClaw）

## 1. 目标与对齐范围
- 让模型在同一会话内可调用“子 agent 会话”，形成可并行、可追踪、可回传的任务链路。
- 对齐 OpenClaw 的 session 工具体系：`sessions_list` / `sessions_history` / `sessions_send` / `sessions_spawn`。
...
## 3. OpenClaw 对齐能力清单（摘要）
- `sessions_list`：列出会话（可带最近消息）。
- `sessions_history`：拉取指定会话历史。
- `sessions_send`：跨会话投递消息，支持 `timeoutSeconds` 等待结果。
- `sessions_spawn`：异步启动子会话，立即返回 `runId + childSessionKey`；默认禁用子会话再 spawn；完成后进行 announce 回传。
- 子会话默认使用“全工具集 - session 工具”，并支持 allowlist 指定 `agentId`。
- sandbox 会话默认只能看到“自己 spawn 的会话”。

## 4. 核心设计（wunder）

### 4.1 会话工具（对齐 OpenClaw 命名）
新增内置工具（`src/services/tools.rs`）：
- `sessions_list`：列出当前用户可见会话（支持 `limit` / `activeMinutes` / `messageLimit`）。
- `sessions_history`：读取某会话历史（`session_id` 或兼容 `sessionKey`）。
- `sessions_send`：向指定会话发送消息；可选 `timeoutSeconds` 等待结果。
- `sessions_spawn`：创建子会话并异步运行，返回 `{status, run_id, child_session_id}`。
...
- `run_id / session_id / parent_session_id / agent_id / model`
- `status / started_at / finished_at / elapsed_s / error`
- `context_tokens / context_tokens_peak / user_rounds / model_rounds`

### 4.3 子会话调度与回传
- `sessions_spawn` 创建子会话记录，立即 `tokio::spawn` 启动 orchestrator。
...
- `sessions_send` 在等待模式下可触发“父-子 ping-pong”轮次。
...
## 6. 权限与安全
- `sessions_spawn` 目标 `agent_id` 受 allowlist 控制（默认仅自身 agent）。
- 子会话默认禁用 session 工具，避免递归 spawn。
...
### 节点 B：Session 工具
- 内置 `sessions_list/history/send/spawn` 工具。
- 完成工具参数校验与权限过滤。
...
## 10. 差异与适配说明
- wunder 已具备 `agent_id` 与多会话，但尚缺 session 工具与内部 spawn。
- OpenClaw 的多智能体 routing 属于网关层能力，wunder 暂以“会话内子 agent”对齐；后续可扩展 bindings 路由层。

---

该方案优先复用现有 orchestrator、monitor 与 chat/session 存储，补齐子会话工具与调度，使功能可在原有架构中最小化接入，并逐步对齐 OpenClaw 的 session 工具体系与子会话运行模型。
