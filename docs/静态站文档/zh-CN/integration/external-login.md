...
- 未传 `agent_name`，或名称未命中已有智能体：`/app/chat`
- 命中已有智能体并进入聚焦模式：`/app/embed/chat`
- desktop 对应 `/desktop/chat` 与 `/desktop/embed/chat`

也就是说，Wunder 不是只返回 token，还会根据是否命中指定智能体决定最终聊天壳。
