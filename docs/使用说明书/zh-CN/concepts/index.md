﻿﻿﻿---
title: 核心概览
summary: 理解 Wunder，先抓住 11 个核心。它们定义的是系统必须守住的设计原则，而非功能清单。
---

# 核心概览

Wunder 面向长期稳定运行、多用户并发、智能体真正执行任务的场景。系统为此建立了 11 个核心设计原则。理解这些核心，有助于在使用 Wunder 时判断"为什么会这样"、"遇到问题时从哪里排查"。

## Wunder 的四条设计哲学

在了解 11 个核心之前，先了解这四条总原则：

**1. 一切围绕线程**

Wunder 以"一条持续运行的线程"为中心，而非"一次问答"。用户与智能体的每一次对话、智能体执行的每一个任务，都发生在线程里。线程是 Wunder 最重要的执行单元。
...
用户在蜂巢里工作，蜂巢有桌面端和网页端两种形态；命令行、飞书微信等也是接入同一套系统的入口。入口不同，能力一致。

**4. 安全与治理优先**

Wunder 默认按多用户、长会话、高风险工具来设计。安全限制、资源配额、操作审计在系统骨架中内置，不靠事后补丁。
...
<div class="docs-card-grid docs-card-grid-compact">
  ...
  <a class="docs-card" href="/docs/zh-CN/concepts/companion/"><strong>形象系统</strong><span>精灵动画形象，让智能体以更生动的方式陪伴工作。</span></a>
  <a class="docs-card" href="/docs/zh-CN/concepts/goal-mode/"><strong>目标态系统</strong><span>设定目标让智能体持续工作直到完成。</span></a>
</div>
...
- 记忆 → 用户的长期资料怎么被使用
- 上下文压缩 → 为什么长对话不会断
- 形象系统 → 让智能体以更生动的方式陪伴工作
- 目标态系统 → 让智能体持续工作直到任务完成
