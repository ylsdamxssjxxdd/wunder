...
- 默认在发起任务范围内复用工蜂任务线程：如果工蜂已有任务线程，任务会直接落到那条任务线程；如果还没有，系统会先创建并绑定一条任务线程。
- 当用户需要强制让工蜂从一个干净线程重新开始时，可以显式传 `threadStrategy=new_thread`。
- `task_thread` 语义是：优先复用工蜂当前任务线程；如果它还没有任务线程，系统会先创建一个再派工。
