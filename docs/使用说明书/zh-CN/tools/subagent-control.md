...
停止主智能体会同时中断其后台和多级子智能体，并保留历史。用户继续聊天后，主智能体先用 `list` 查看，再按需 `send(session_id,message)` 或 `resume(session_id,message)` 恢复工作或分派新任务。未被选中的子智能体保持待命。`resume` 不带消息只重新开放线程；运行中的子任务应等待中断收敛或完成后再分派。
