...
`dry_run` 与正式执行共用暂存、目标冲突检查和上下文匹配逻辑，通过后丢弃暂存结果，不写磁盘。以下情况优先使用：
...
用户界面和管理员结果保留 `files[].diff_blocks` 预览：每文件最多 80 行，单次调用合计最多 320 行、24 KiB 行正文。超出剩余字节预算的行会整行省略，已展示行的内容与原始行号保持准确。结果和文件层的 `added_lines/deleted_lines` 是实际增删行数，`diff_lines_omitted` 是省略的预览行数。此限制同样适用于 `dry_run`，只影响展示，不缩减实际修改；预览不能作为被删内容的备份。

模型只接收文件摘要和准确的变更计数，不接收 diff 正文。现有工具日志保留调用及有界结果，便于追溯。
