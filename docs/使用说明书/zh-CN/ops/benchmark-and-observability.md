...
服务版 `wunder-server` 会额外把运行日志写到本地 `config/data/logs/server/`。

- 只有 server 形态写本地日志，desktop 和 cli 不会写这里。
- 文件是按天轮转的 JSONL，默认保留 14 天，便于直接 grep、归档或接入采集系统。
- 当服务异常退出时，优先先看最新日志里的 `panic`、`server exited unexpectedly`、HTTP 5xx / failure 记录。
...
- `round_usage.total_tokens`：实际上下文占用
