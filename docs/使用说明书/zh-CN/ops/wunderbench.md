...
运行概览只展示状态、总分、结论、评测目标和耗时；任务执行、评分与工具调用的完整诊断信息请在执行记录中打开对应 attempt，或导出评测记录查看。

页面上主要有四类操作：

- **选择题库**：可选择内置题库或已导入的版本化题库；每次运行只执行所选题库的全部可用任务。
- **导入题库**：上传可信来源的 `.zip` 题库包。含自动评分脚本的题库会要求明确确认，因为脚本会在评测阶段由本机 Python 执行。
- **选择预设智能体**：可选择默认智能体或已配置的预设智能体，以其系统提示词、工具、技能预览和沙盒容器执行评测。
- **选择被测模型**：真正执行任务的模型；显式选择时会覆盖预设智能体配置的模型。
...
选择 **预设智能体** 后，WunderBench 会在启动时读取其当前配置，并将下列内容写入运行快照：系统提示词、工具与技能声明、技能预览开关、沙盒容器、预设模型、预设标识和版本。运行中的配置不会再受后续预设编辑影响，导出记录可以据此复盘一次测试。

预设智能体用于评测配置本身，不会被当作待测用户的实际智能体记录绑定到评测线程。若不选择预设智能体，WunderBench 保持原有的仅模型评测行为。

模型选择优先级如下：

1. 在页面或 `model_name` 中显式指定的被测模型。
2. 所选预设智能体配置的模型。
3. 系统默认模型。

预设智能体会提供其配置的工具集合；API 请求显式传入非空 `tool_names` 时，以该字段覆盖预设工具，便于进行受控的工具回归测试。
