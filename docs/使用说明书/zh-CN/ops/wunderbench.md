...
summary: 使用三档评测自动验证模型在 Wunder 真实智能体链路中的任务完成质量，并为题库扩展、回归检测和模型选型提供依据。
read_when:
  - 你需要评估模型在 Wunder 中的真实任务完成能力
  - 你需要导出完整评测记录，复盘模型日志、工具调用和工作区结果
  - 你准备为 WunderBench 新增或调整评测题目
source_docs:
  - docs/API文档.md
...
WunderBench 默认只保留三个档位，便于记忆和操作。

| 档位 | 用途 | 选择规则 | 推荐次数 |
|------|------|----------|----------|
| `quick` | 快速冒烟，确认模型和工具链路是否可用 | 每个任务组选一个优先级最高的轻量任务 | 1 次 |
| `core` | 常规回归，覆盖主要能力面 | 在 quick 基础上补充更多关键任务，最多覆盖当前核心题库 | 2 次 |
| `full` | 发布前或深度对比 | 运行全部可用任务 | 2 次 |

建议用法：

- 日常开发改动后先跑 `quick`，确认链路没有明显故障。
- 调整模型、提示词、工具策略后跑 `core`，看能力是否有退化。
- 发布前、重要模型切换前、题库大改后跑 `full`，并导出记录留档。
...
| `name` | 页面展示名称 |
| `suite` | 任务组，用于 quick/core/full 选择与弱项聚合 |
| `category` | 任务分类 |
...
- **成本可控**：单题不要过长，避免 `quick` 和 `core` 被少数大题拖慢。
