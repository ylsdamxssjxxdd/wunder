...
| `WUNDER_SERVER_FEATURES` | Docker 下 Rust 服务编译特性，默认 `mcp,host-metrics` |
| `WUNDER_SANDBOX_DOCKER_READ_ONLY` | Docker Compose 下 sandbox 容器级只读根文件系统开关，默认 `false` |

### Docker 下的系统状态

管理员侧系统状态中的 CPU、内存、进程、负载和磁盘指标依赖 `host-metrics` 编译特性。当前 Compose 默认使用 `WUNDER_SERVER_FEATURES=mcp,host-metrics`；如果在 `.env` 中手动覆盖该变量，需要保留 `host-metrics`，否则这些主机资源指标会以 0 值降级显示。
...
| 管理端系统状态资源指标为 0 | `WUNDER_SERVER_FEATURES` 覆盖后缺少 `host-metrics` | 恢复为 `mcp,host-metrics` 并重建 server/sandbox |
