...
这个形式适合你不想在外部系统里先生成 JWT，而是直接把用户 ID 带进 Wunder 的场景。
...
如果你需要嵌入态智能体页，也可以继续带 `section=agents&agent_id=<agent_id>` 进入，同样不会显示左侧栏和中栏。Wunder 不是只返回 token，还会根据是否命中指定智能体决定最终主内容状态。
