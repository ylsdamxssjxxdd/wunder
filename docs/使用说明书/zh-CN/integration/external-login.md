...
- 未传 `agent_name`，或名称未命中已有智能体：`/app/chat?section=messages&entry=default`
- 命中已有智能体并进入聚焦模式：`/app/embed/chat?section=messages&agent_id=<agent_id>`
- desktop 对应 `/desktop/chat?section=messages&entry=default` 与 `/desktop/embed/chat?section=messages&agent_id=<agent_id>`

也就是说，普通聊天页至少会带 `section=messages`；默认智能体会额外带 `entry=default`，聚焦模式则会带命中的 `agent_id`。
Wunder 不是只返回 token，还会根据是否命中指定智能体决定最终聊天壳。
