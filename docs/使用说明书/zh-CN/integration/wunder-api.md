...
如果用户要做会话列表、恢复、取消、实时工作台，建议直接进入聊天域：`/wunder/chat/*`。
...
如果用户绕过这些预处理，自己直接调 `/wunder`，那附件拆解、转写、抽帧和体积控制都要由接入方自己负责。
