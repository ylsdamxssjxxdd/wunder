...
系统中的多张表共同支撑边界处理与稳定性保障机制：

| 表名 | 边界处理职责 | 关键字段 |
|------|-------------|---------|
| `stream_events` | 事件回放与断线恢复 | `event_id`（递增时间线）、`after_event_id`（续传断点）、`event_type`（context_compaction/tool_failed/queue_position等） |
| `agent_threads` | 线程状态管理与恢复 | `status`（idle/running/waiting_approval/waiting_user_input/system_error）、`last_state_update_at` |
| `agent_tasks` | 排队任务管理 | `status`（pending/running/retry/success/failed/dead）、`priority`、`retry_count` |
...
这些表共同实现：
- 事件持久化与断线续传（stream_events）
- 会话锁与超时管理（session_locks）
- 线程状态与恢复（agent_threads）
- 队列任务与重试（agent_tasks）
- 上下文压缩历史记录（monitor_sessions/chat_sessions）
