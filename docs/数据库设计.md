...
```mermaid
flowchart LR
  RUN[benchmark_runs] --> ATTEMPT[benchmark_attempts]
  RUN --> TASK[benchmark_task_aggregates]
...
benchmark_runs
- 用途。PinchBench 风格能力评估主表。
- 关键字段。run_id、user_id、model_name、judge_model_name、status、total_score、started_time、finished_time、payload。
- 索引。idx_benchmark_runs_user，idx_benchmark_runs_status，idx_benchmark_runs_started。

benchmark_attempts
- 用途。单任务多轮 attempt 明细。
- 关键字段。id、run_id、task_id、attempt_no、status、final_score、started_time、finished_time、payload。
- 索引。idx_benchmark_attempts_run，idx_benchmark_attempts_status。

benchmark_task_aggregates
- 用途。任务聚合结果。
- 关键字段。id、run_id、task_id、status、mean_score、payload。
- 索引。idx_benchmark_task_aggregates_run。
