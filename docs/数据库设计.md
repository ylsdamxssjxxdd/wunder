...
| 用户 | `user_accounts` / `user_tokens` / `org_units` | 账号、Token 与单位结构。 |
...
索引：
- `idx_session_locks_user_agent`（`user_id`, `agent_id`）
...
| status | TEXT | 是 | 状态。 |
| access_level | TEXT | 是 | 访问级别（保留字段，统一为 A）。 |
| unit_id | TEXT | 否 | 所属单位 ID。 |
...
- `idx_user_accounts_unit`（`unit_id`）
...
| 字段 | 类型 | 必填 | 说明 |
| --- | --- | --- | --- |
| unit_id | TEXT | 是 | 主键，单位 ID。 |
| parent_id | TEXT | 否 | 上级单位 ID。 |
| name | TEXT | 是 | 单位名称。 |
| level | INTEGER | 是 | 层级（1~4）。 |
| path | TEXT | 是 | 路径 ID 串。 |
| path_name | TEXT | 是 | 路径名称串。 |
| sort_order | BIGINT | 是 | 同级排序。 |
| leader_ids | TEXT | 是 | 负责人用户 ID 列表 JSON。 |
| created_at | DOUBLE PRECISION | 是 | 创建时间。 |
| updated_at | DOUBLE PRECISION | 是 | 更新时间。 |

索引：
- `idx_org_units_parent` (`parent_id`)
- `idx_org_units_path` (`path`)
...
- 会话锁以 `session_id` 互斥，`user_id + agent_id` 索引用于快速定位运行中的会话。
