...
- Throughput testing (`src/throughput.rs`): simulate concurrent users, randomize questions, persist reports for replay/export.
...
- Admin-triggered concurrency benchmarks with randomized questions and duration controls.
...
- Workspace: `workspaces/<user_id>/files` (prompt shows `/workspaces/<user_id>/`), isolated and persistent.
