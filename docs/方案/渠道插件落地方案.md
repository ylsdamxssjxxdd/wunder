...
> 2026-03-02 对齐说明：
> - 当前主干已完成“渠道适配器注册表”重构（`ChannelAdapter/Registry/Catalog`），文中关于 `deliver_outbox_record` 硬编码分支、`SUPPORTED_USER_CHANNELS` 静态列表等问题已部分解决。
> - 本文档仍可作为“进程插件化”长期方案，但建议按分阶段推进（先桥接 `ChannelAdapter`，再做插件安装与生命周期管理）。
