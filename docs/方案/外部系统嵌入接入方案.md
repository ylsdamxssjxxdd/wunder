...
> 目标：把 **用户侧前端（`frontend/`）** 作为外链嵌入到其他系统页面中，并复用对方的鉴权/用户体系；外部系统提供 `key + 账号 + 单位(可选)` 即可让用户“免登录进入 wunder”，若 wunder 内不存在该用户则自动创建。
...
- 或配置文件：`security.external_auth_key: ${WUNDER_EXTERNAL_AUTH_KEY:-}`（未显式配置时会自动回退到 `security.api_key`）

> 未配置时 `/wunder/auth/external/*` 视为禁用。

2) 配置外链预制智能体（只允许进入这一类）：

- 环境变量：`WUNDER_EXTERNAL_EMBED_PRESET_AGENT_NAME=文稿校对`
- 或配置文件：`security.external_embed_preset_agent_name: 文稿校对`

说明：
- 外链启动仅允许进入该预制智能体名称。
- 若目标用户下存在“同名用户智能体”，优先进入用户同名智能体（不强制覆盖为预制记录）。

### 2.2 外部系统后端：一键启动嵌入入口（推荐）

外部系统后端在用户已登录的前提下，拿到用户信息后调用：

- `POST /wunder/auth/external/launch`
- Body（JSON）：`key`、`username`、`unit_id`（可选）、`password`（可选，兼容旧调用）

说明：
- `username` 会被归一化为 wunder 的 `user_id`（仅允许 `[A-Za-z0-9_-]`）。
- 若用户不存在：自动创建并绑定单位（未传 `unit_id` 则使用默认根单位）。
- 若用户已存在：默认不改密码（更安全）；仅在显式传 `password` 时兼容旧逻辑；传了 `unit_id` 且不同则更新单位（同时按单位层级默认额度规则做最小化调整）。
- 返回 `data.entry_path`（如 `/app/embed/chat?wunder_code=...&agent_id=...&embed=1`）。
- 默认一次性码 TTL=60 秒，且 **单次使用**（一次性码存内存，服务重启会失效）。

### 2.3 外部系统前端：iframe 嵌入

外部系统后端把 `entry_path` 渲染到页面中，iframe 直接指向 wunder 用户前端：

```html
<iframe
  src="https://<wunder-frontend-host><entry_path>"
...
4) 进入 `/app/embed/chat` 嵌入聊天壳，默认隐藏左侧栏与中栏，仅保留聊天主区。
