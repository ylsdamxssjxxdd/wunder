...
## 3.4 本次演示验收要求（2026-03-12 明确）

以下要求为本轮蜂群演示改造的强约束，后续实现与测试必须逐条满足，不能用“近似效果”替代：

1. 用户在蜂群页右侧栏选中的智能体，就是本次演示的母蜂。
2. 不允许继续复用历史母蜂记录覆盖当前选择；无论之前谁当过母蜂，本次都以当前右侧选择为准。
3. 默认智能体（`__default__`）必须可作为母蜂参与演示，不能因为它是默认快照或非持久化记录而被回退到其他智能体。
4. 点击“演示”后，必须由所选母蜂先接收演示消息并开始模拟输出。
5. 母蜂首次输出结束后，必须触发蜂群工具，将任务派发给工蜂。
6. 母蜂派发后必须继续产生一次模拟输出，并显式进入等待状态；不能在派发后立即结束。
7. 工蜂必须并发开始作业，并各自产生模拟输出与相应任务执行过程；不能全部瞬间完成。
8. 所有工蜂结束后，结果统一回传给母蜂，再由母蜂输出最终汇总内容。
9. 演示过程中必须能看到状态推进，不能依赖手动刷新后才看到结果。
10. 演示链路必须稳定创建并关联 `team_run`；出现 `demo team run was not created` 视为未达标。

### 3.4.1 目标演示时序

1. 右侧栏选择一个智能体，例如“默认智能体”。
2. 点击“演示”。
3. 所选智能体成为母蜂并接收演示消息。
4. 母蜂开始第一段模拟输出。
5. 母蜂触发蜂群工具，按输出内容调用工蜂。
6. 母蜂继续第二段模拟输出，并触发等待。
7. 工蜂并发执行，各自完成模拟输出与任务。
8. 全部工蜂完成后，结果回传母蜂。
9. 母蜂输出最终汇总结果。

### 3.4.2 禁止出现的错误表现

1. 无论选择哪个智能体，母蜂始终固定为某个历史智能体（例如“数据分析”）。
2. 演示启动后瞬间结束，没有母蜂等待和工蜂并发阶段。
3. 工蜂没有实际任务过程，只出现极短的收尾消息。
4. 时间线或状态必须手动刷新才更新。
5. `team_run` 未创建、未关联或无法追踪。
