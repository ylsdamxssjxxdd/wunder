> 状态：已全部实施完成并归档（2026-09-25）。第 8 章"已落地"为验收记录；文中"现状判断"描述的是重构前状态，现行结构见 docs/技术说明书/02-仓库结构与模块职责.md。
# 后端轻量化重构
...
- `wunder-core` 已作为轻量稳定边界建立，并由 `wunder-runtime` 显式依赖与导出；现已承载配置模型、公共 DTO/schema、鉴权路径规则、token/JSON schema/工具参数归一、i18n 纯翻译规则、请求限制、原子写入、LLM 速度统计、执行策略纯决策、存储基础记录、桥接查询参数、存储常量与 `StorageBackend` 抽象。`wunder-runtime` 仅保留 axum/tokio/tracing/数据库/平台 API/进程级状态相关适配和兼容 re-export。
- 已将低依赖的仓库资源路径解析逻辑迁入 `wunder-core::repo_assets`，runtime 保留同名 re-export 兼容旧调用点，使 core 开始承载真实稳定基础能力。
- 已将 `request_limits`、`core::approval` 纯枚举、`core::atomic_write`、`core::i18n` 纯规则、`core::llm_speed`、`core::exec_policy` 纯决策、`storage::{records,bridge,constants,backend}` 迁入 `wunder-core`；runtime 侧保留当前语言 task-local、审批通道、工具名解析、审批缓存、HeaderMap 适配、存储实现与构建工厂，避免把 Tokio、axum、tracing、数据库客户端或平台专属依赖带入 core。
- 本批已通过 `cargo test -j 8 -p wunder-core`、`cargo check -j 8 -p wunder-runtime --lib`、`cargo check -j 8`、`cargo check -j 8 -p wunder-cli`、`cargo check -j 8 -p wunder-desktop --bin wunder-desktop-bridge` 与 `cargo check -j 8 --workspace`；`wunder-core` 常规依赖保持为 `anyhow`、`chrono`、`regex`、`serde`、`serde_json`、`serde_yaml`，`tempfile` 仅作为测试依赖。
...
- 已将 `wunder-core::storage_backend::StorageBackend` 从单一大 trait 拆为 `StorageLifecycle`、`MetaStore`、`ConversationLogStore`、`LogStatsStore`、`MonitorStore`、`SessionLockStore`、`AgentRuntimeStore`、`VectorDocumentStore`、`MemoryRecordStore`、`BenchmarkStore`、`RetentionStore`、`UserAccountStore`、`ChatSessionStore`、`SessionGoalStore`、`UserWorldStore`、`BeeroomStore`、`ChannelDirectoryStore`、`ChannelRuntimeStore`、`BridgeStore`、`GatewayStore`、`MediaStore`、`SessionRunStore`、`CronStore`、`AgentDirectoryStore`、`TokenBalanceStore` 等领域子 trait，并保留组合 `StorageBackend` 作为现有 `Arc<dyn StorageBackend>` / `&dyn StorageBackend` 调用入口。
- SQLite/Postgres 的 `backend_impl.rs` 已改为分别实现上述领域子 trait，仍只转发到既有领域模块的 `*_impl` 方法，不改变 SQL、事务、feature gate 或返回结构；具体 `SqliteStorage`/`PostgresStorage` 直接调用存储方法时改为显式导入所需领域 trait，后续服务可逐步从总 `StorageBackend` 依赖收窄到领域接口。
- 已建立 `core::blocking` 作为唯一受控阻塞执行入口，按 DB/FS/CPU/External 分类提供并发预算、排队超时、执行超时和 tracing 字段；cron、channels persistence/outbox/bridge、workspace、auth、memory、orchestrator、runtime mission/thread、API 文件操作、工具执行等高频 `spawn_blocking` 路径已迁移到分类入口，生产代码搜索中裸 `spawn_blocking` 仅保留在统一入口内部。
- 已建立 `core::bounded_queue` 作为有界队列 helper，提供 bounded channel、限时入队和带并发上限的 dispatcher；runtime/server 范围内 `UnboundedSender`、`UnboundedReceiver`、`unbounded_channel` 与 `UnboundedReceiverStream` 已清空，channels、approval、admin knowledge 流、stream persist 等队列路径改为显式容量、try-send 降级或超时入队。
- 已建立 `core::long_task` 作为后台长任务入口，提供统一 label、启动/完成 tracing 和超阈值 warn；cron scheduler/heartbeat/manual/timer、channels supervisor/worker/outbox/bootstrap/approval forward、runtime thread/mission 队列与执行、tools session run/command reader、orchestrator stream pump/heartbeat/memory auto extract、A2A/API 后台 job、WS 业务 stream/resume/watch、desktop LAN、MCP SSE reader、sim lab 等长生命周期入口已迁移，生产裸 `tokio::spawn` 保留范围收敛为 WebSocket writer 泵和测试 fixture。
- 本批运行时边界治理已验证 `cargo fmt`、`cargo check -j 8 -p wunder-runtime --lib`、`cargo check -j 8 -p wunder-server`、`cargo check -j 8 -p wunder-runtime --features sqlite-storage --no-default-features --lib`、`cargo check -j 8 -p wunder-runtime --features postgres-storage --no-default-features --lib`，并通过 `blocking`、`bounded_queue`、`long_task` 三组核心单测；最终 workspace 检查作为本轮收口验收项保留。
- 已在 `core::runtime_metrics` 建立运行时边界内存聚合，直接从 `core::blocking`、`core::bounded_queue`、`core::long_task` 记录调用量、in-flight、排队/执行最大耗时、超时、busy、失败、长任务 warn/panic 和阈值告警；`/wunder/admin/monitor` 与 `/wunder/admin/runtime_metrics` 均可读取同一份快照。
- 管理端监控页已新增“运行时边界”卡片，展示阻塞执行、有界队列、长任务三类汇总和告警列表；`scripts/runtime_boundary_stress.py` 已扩展为场景化压测入口，默认覆盖 monitor 与 channels 只读路径，并可显式开启 cron、performance、channel-probe 与 WS 长链路场景，对比前后快照输出 blocking/queue/long_task 增量、热点 label 与告警。
- 场景边界按风险分层：`all-readonly` 仅触发 monitor/channels/cron 查询，`performance` 通过 `/wunder/admin/performance/sample` 覆盖 orchestrator/runtime/tool/file/command/vector/log 热点，`channel-probe` 需要显式 channel 参数才写入探针日志，`ws` 需要用户 token 或 core ws 的 API key + user_id，并在缺少 Python `websockets` 依赖时给出明确安装提示。
