...
### 相关改动
- 缓存定义与构建：src/services/tools.rs:75
- 缓存接入工具上下文：src/orchestrator/execute.rs:82
- 性能采样复用缓存：src/ops/performance.rs:7
...
### 相关改动
- 沙盒入口：`src/sandbox/server.rs`
- 服务模式切换：`src/main.rs` + `src/core/config.rs`
- 部署配置：`docker-compose-x86.yml`、`docker-compose-arm.yml`

### 指导与注意事项
- 沙盒容器需设置 `WUNDER_SERVER_MODE=sandbox` + `WUNDER_PORT=9001`，并确保 `WUNDER_SANDBOX_ENDPOINT` 指向容器内地址（默认 `http://wunder-sandbox:9001`）。
