...
Docker 运行（无需本地安装 k6）：
```
docker run --rm -i -e WUNDER_API_KEY=你的密钥 -v "${PWD}:/work" -w /work grafana/k6 run -e WUNDER_PROFILE=load -e WUNDER_BASE_URL=http://192.168.0.2:18000/wunder tests/performance/k6_wunder.js
```

### 4.3 可选参数（环境变量）
- `WUNDER_PROFILE`：压测档位，`quick/load/spike/soak`，默认 `load`
- `WUNDER_BASE_URL`：默认 `http://127.0.0.1:18000/wunder`
...
示例：
```
$env:WUNDER_API_KEY="你的密钥"; $env:WUNDER_BASE_URL="http://127.0.0.1:18000/wunder"; $env:WUNDER_PROFILE="load"; k6 run tests/performance/k6_wunder.js
```
...
## 8. Benchmark 测试（PinchBench 对齐）
### 8.1 用例说明
- `config/benchmark/tasks/*.md`：Benchmark 任务定义，需校验 Markdown frontmatter、必备分段、自动评分块与 LLM 裁判 rubric 。
- 统一通过 benchmark 单测与管理端 `/wunder/admin/benchmark/*` 链路验证，运行 ID 和工作区均为隔离生成。
...
### 8.3 输出说明
- Benchmark 结果默认持久化到 `benchmark_runs` / `benchmark_attempts` / `benchmark_task_aggregates`。
- 若通过管理端发起，可在能力评估面板直接查看 SSE 进度、历史运行与 attempt 明细。
