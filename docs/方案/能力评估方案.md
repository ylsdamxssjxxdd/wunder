...
> 本文档对应旧的 `/wunder/admin/evaluation/*` 题库式能力评估设计，当前已不再作为系统主链路。
>
> 现行实现请参考 `docs/方案/PinchBench能力评估落地方案.md`，并统一使用 `/wunder/admin/benchmark/*` 接口。
...
3. 逐条执行：
   ...
   - 运行 checker 判定，产出 `score`、`status` 与 `detail`（`score/max_score` 为按维度权重分摊后的分值）。
4. 汇总评分：按维度权重生成总分与摘要。
...
- 旧 `/wunder/admin/evaluation/*` 接口已移除。
- 当前统一使用 `/wunder/admin/benchmark/*`：`suites`、`tasks`、`start`、`runs`、`runs/{run_id}`、`runs/{run_id}/cancel`、`runs/{run_id}/stream`。
- benchmark 运行支持 suite / task / attempt 三层结构，以及 `automated` / `llm_judge` / `hybrid` 三种评分模式。
