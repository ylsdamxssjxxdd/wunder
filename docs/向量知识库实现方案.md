...
- 向量知识库存储在新目录（建议 `vector_knowledge/`，用户侧 `vector_knowledge/users/<user_id>/`），避免使用 `data/`。
...
### 3.3 数据模型与元数据
建议分层：
- **本地文件**：保存原始文件与解析后的 Markdown（便于展示与再切片）。存储根目录建议 `vector_knowledge/`，其中 `shared/<base>` 与 `users/<user_id>/<base>` 分离。
- **元数据（Postgres）**：
  - `vector_knowledge_bases`：base_id/name/type/embedding_model/chunk_size/overlap/owner_id/enabled/shared
  - `vector_knowledge_documents`：doc_id/base_id/file_path/hash/status/chunk_count/embedding_model
- **向量数据（Weaviate）**：
...
3. 生成 `doc_id`（文件 hash）与 `chunk_id`（doc_id + chunk_index + content_hash）。
4. 逐批请求嵌入模型（/v1/embeddings），写入 Weaviate。
5. 存储元数据（文档状态、chunk 数量、embedding_model），便于 UI 展示。

### 3.5 检索与工具挂载
- 向量知识库工具与字面库共用 `query/limit` 入参；真正检索配置（top_k、score_threshold）写在 base 配置内。
- 工具执行时：
...
### 3.6 可视化与可控性
- 文档视图：原文/Markdown 预览。
- 切片视图：按 chunk_index 展示列表，支持点击高亮原文区间。
- 管理操作：重新解析、重新索引、删除文档、批量刷新。
- 索引状态：`pending/indexing/ready/failed`，失败可重试。
...
- `GET/POST /wunder/admin/knowledge` 与 `/wunder/user_tools/knowledge`
  ...
  - `type=vector` 时要求 `embedding_model`，root 自动指向 `vector_knowledge/...`
- `POST /wunder/*/knowledge/upload`
  - 字面库：保持原行为
  - 向量库：doc2md → 切片 → 向量入库，返回 `doc_id/chunk_count/status`
- 新增：
...
- 向量库文档区：显示 doc 状态、chunk 数量；右侧展示 chunk 列表与原文高亮。
