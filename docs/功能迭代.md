...
- [desktop][backend][frontend] desktop 远端接入一期落地：启动时按 remote_gateway 自动 login/register 远端角色，连接成功后 runtime 切换远端 api_base/ws_base/token；本地桌面设置接口改用 desktop_token 独立鉴权。
...
### 文档
- [desktop][docs][api] 更新 desktop API/设计/系统介绍与实现方案，补充 remote_gateway 一期行为（双 token、远端握手与失败回退）及新增 runtime 字段说明。
