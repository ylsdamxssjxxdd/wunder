...
- [docs][架构设计] 部署拓扑图调整 wunder_server 与向量数据库节点宽度，确保文字完整显示
...
- [docker] wunder_server 启动前检查目标二进制是否过期，等待 sandbox 重新编译后再启动
...
- [docker] wunder_server 不再等待 sandbox 健康检查，改为 service_started 以适配首次编译耗时
