...
- [backend][docker][config] docker compose 开箱即用：wunder-mcp 加入 build 并为本地镜像设置 pull_policy: never，避免首次启动在镜像未构建完成时拉取失败；同时 config/wunder.yaml 缺失时自动回退 config/wunder-example.yaml，允许无 .env 直接启动。
...
- [frontend] 聊天wunder-mcp广场调整为 /home 并移除临时文件区入口
