...
```
docker run --rm -i -v "${PWD}:/work" -w /work grafana/k6 run -e WUNDER_PROFILE=load tests/performance/k6_wunder.js
```
...
Example:
```
$env:WUNDER_BASE_URL="http://127.0.0.1:8000/wunder"; $env:WUNDER_PROFILE="load"; k6 run tests/performance/k6_wunder.js
```
