...
当前代码对齐的全量表为 59 张（以 `crates/wunder-runtime/src/storage/sqlite/schema.rs` 与 `postgres/schema.rs` 为准）：
...
- 线程排队模型由 `agent_tasks`（含 `thread_id` 列）+ ThreadRuntime 承载，无独立线程状态表。
...
| `agent_tasks` | 排队任务管理（含线程归属 `thread_id`） | `status`、`priority`、`retry_count` |
| `session_locks` | 会话互斥与超时 | `session_id`、`user_id`、`agent_id`、`expires_at` |
| `chat_sessions` | 会话状态持久化 | `parent_session_id`、`spawned_by`、`last_message_at` |
| `monitor_sessions` | 监控与可观测性 | `status`、`updated_time`、`payload`（token 统计在 payload JSON 内） |
| `model_context_entries` | 上下文压缩与模型上下文状态 | 以条目形式记录模型上下文演变（见 `docs/设计文档/上下文压缩设计.md`） |
...
切片级内容另有 `vector_chunks` 表（按 chunk 落库，支撑切片级向量检索），不再只依赖 `chunks_json` 字段。
...
retention 调度器已降级为兼容 no-op（`storage/sqlite/retention_store.rs`），不再按 `retention_days` 自动清理。日志默认永久保留，删除只通过管理员日志清理接口显式执行；用户侧普通线程清理与管理员日志删除明确分离。详见 `docs/技术说明书/06-数据存储、工作区与配置.md`。
...
1. `crates/wunder-runtime/src/storage/sqlite/schema.rs` 与 `postgres/schema.rs` 的建表或补列逻辑（领域读写分散在各 store 文件中）。
