...
### 1.1 工具提示词注入（wunder 行为）
- `tool_call`：系统提示词会注入工具清单 `<tools>...</tools>` 与调用规则，并追加 A2UI 引导提示。
- `function_call`：系统提示词不再注入工具清单与工具调用引导，保持基础系统提示与工程信息，工具清单仅通过请求的 `tools` 传入。
- 技能提示词在两种模式下都会注入（基于启用与 `tool_names` 过滤），确保模型知道技能的用法与执行规则。
...
### 3.2 执行与续写
系统读取 `tool_calls`，执行工具后将结果以 role=tool + tool_call_id 的消息追加到上下文，再继续请求模型生成最终答案。

### 3.3 历史回填（标准对齐）
- `function_call` 模式下，assistant 的 `tool_calls` 与对应的 role=tool/tool_call_id 结果会写入对话历史，并在后续请求中原样回填，以满足 OpenAI tools 协议的多轮要求。
- `tool_call` 模式继续使用 `tool_response: ` 前缀的 user 消息回填。
...
sequenceDiagram
  ...
  O->>T: execute tool
  T-->>O: tool_result
  O->>L: role=tool + tool_call_id 追加到上下文
  L-->>O: content (最终答案)
  O-->>U: answer
```
...
工具调用链路里存在多层裁剪，排障时务必区分“会影响模型输入”与“仅影响展示”：

1. 工具自身裁剪：会影响模型输入
2. 编排层全局裁剪（`finalize_tool_result`）：会影响模型输入
3. 模型观测压缩（`model_observation`）：只影响模型 observation，不改原始 `tool_result.data`
4. 监控/时间线裁剪：影响监控记录与时间线接口，不直接改模型输入
5. 前端显示裁剪：仅 UI 展示，不改后端真实数据

排障建议：

1. 先看 `tool_result.data`（真实工具回包）
2. 再看 `model_observation`（实际送入模型）
3. 最后看时间线/前端显示层是否有二次裁剪

详细链路与阈值见：`docs/方案/工具结果裁剪与观测链路.md`。
