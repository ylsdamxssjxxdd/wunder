...
| 组成 | 说明 | 主要目录 |
| --- | --- | --- |
| 内核工具治理 | tool spec、approval、sandbox、retry、ledger、surface | `crates/eva/src/tools` `src/agent_kernel/tools` |
| Wunder 工具适配 | 宿主工具面构建、动态工具执行、兼容投影 | `crates/eva/src/wunder/tools` |
| 业务工具实现 | 文件、shell、browser、desktop、memory、channel 等 | `src/services/tools` `src/domain_tools` |
| 用户工具与预设 | 用户侧启用、限制、预设和工具目录 | `src/services/user_tools.rs` `src/services/default_tool_profile.rs` |
| Skills | 流程化能力、仓库化技能内容 | `src/services/skills.rs` `skills` |
