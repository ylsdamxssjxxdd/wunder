...
- `goal action=get`：读取当前目标，不改写任何状态。
- `goal action=create`：仅用于创建新目标；如果当前线程已有非终态目标，必须拒绝。
- `goal action=update`：只允许把当前目标标记为 `complete`。
...
1. 解析并校验目标正文。
2. 校验当前线程实际挂载工具中包含 `goal`；若未挂载则拒绝进入。
3. 写入或更新会话目标记录。
4. 通过 `stream_events` 发送 `goal_updated` 事件。
5. 在会话运行态中注册当前目标，保证下一次 turn 能读取到它。
6. 如果当前线程空闲，立即调度一次隐藏的目标续跑 turn。
...
- `reset_work_state` 必须停止当前活跃目标的自动续跑，并让新任务线程从空目标开始。
...
- `crates/wunder-cli/slash_command.rs`：新增 `Goal` 命令。
- `crates/wunder-cli/tui/app/commands.rs`：命令分发与目标面板交互。
