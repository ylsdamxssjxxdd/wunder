...
- extra_mcp PPT tools: 独立 MCP 服务内置 `ppt_write` / `ppt_refine` / `ppt_read` / `ppt_template_read` / `ppt_delete`，使用豆包式页面 XML/JSON 生成可编辑 PPTX。`ppt_write` 创建或追加页面并返回 `presentation_id` 与 `slide_id`；`ppt_refine` 基于保存的 manifest 按 `slide_id` 重渲染指定页面，并可通过 `template_id` 整体切换模板风格；`ppt_read` 与 `ppt_template_read` 返回页面文字/结构摘要，`ppt_template_read` 空参数返回内置模板列表；`ppt_delete` 删除页面并重渲染。内置模板包括 `amber_clear`、`executive_green`、`research_blue`、`finance_ink`、`creative_coral`、`minimal_gray`、`doubao_radar`，其中 `doubao_radar` 复刻豆包相控阵雷达类技术介绍风格。页面可通过 XML `<image src="/workspaces/.../image.png" />` 或 JSON `images` 数组插入本地/工作区位图。产物默认写入 `/workspaces/.extra_mcp/ppt/<presentation_id>/`，也可通过 `output_path=/workspaces/{user_id}/exports/report.pptx` 直接写入工作区。新增或升级工具后，管理端需刷新 `extra_mcp` 的 MCP 工具规格；若配置了 `allow_tools` 白名单，需要包含上述工具名。
