...
## 2026-01-24 沙盒命令执行并发优化

### 背景
- command_exec 在 300 并发场景存在明显排队，主要耗时集中在沙盒服务的线程池与请求调度。

### 优化方案
- 增加沙盒服务线程池可配置项 WUNDER_SANDBOX_THREADPOOL_WORKERS，提升 to_thread 并发处理能力。
- 允许通过 WUNDER_SANDBOX_WORKERS 调整 uvicorn worker 数量，缓解高并发排队。

### 指导与注意事项
- 推荐将 WUNDER_SANDBOX_THREADPOOL_WORKERS 设置为 CPU 核数的 2-4 倍，再观察 command_exec 指标变化。
- WUNDER_SANDBOX_WORKERS 调整会增加进程数量，注意 CPU 与内存占用。
- 指标验证建议使用管理端性能面板或接口：/wunder/admin/performance/sample。

## 2026-01-24 沙盒命令执行异步化

### 背景
- command_exec 在高并发场景仍存在排队延迟，主要受沙盒服务线程池并发上限影响。

### 优化方案
- 在沙盒服务内新增 execute_payload_async，命令执行走 asyncio 子进程，避免每次请求都占用线程池。
- 保持其他工具仍走线程池，避免阻塞事件循环，同时 command_exec 具备更高并发吞吐。

### 相关改动
- 异步入口：app/sandbox/runner.py
- 命令异步执行：app/tools/builtin.py
- 沙盒服务接入：app/sandbox/service.py

### 指导与注意事项
- command_exec 负载高时优先使用异步执行路径，减少线程池排队时间。
- 其他同步工具仍依赖线程池，WUNDER_SANDBOX_THREADPOOL_WORKERS 仍需根据负载调优。
- 指标验证建议使用管理端性能面板或接口：/wunder/admin/performance/sample。
