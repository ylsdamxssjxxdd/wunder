...
- 管理员侧 Web：原“用户管理”更名为“线程管理”，新增“用户管理”面板用于实际用户管理（删除用户/重置密码/权限控制），复用现有管理端模块设计。
- 支持外部系统 iframe 嵌入免登录：后端通过 `security.external_auth_key` + `/wunder/auth/external/*` 对接外部账号体系，前端支持 `wunder_code`/`wunder_token` 引导参数完成自动登录。

## 节点与范围
1) 账号与认证
- 注册/登录/演示登录/获取当前用户。
- 账号状态（active/disabled）、所属单位、角色（user/admin/super_admin）。
- 额度治理：按单位层级默认每日额度（一级/二级/三级/四级 = 10000/5000/1000/100），0 点重置；每次模型调用计入额度，超额提示并阻止继续请求。
- Token 鉴权：Bearer Token（用户端），API Key（管理员端）。

2) 会话与聊天
- 会话创建/列表/详情/删除。
- 发送消息（WebSocket 优先，SSE 兜底），取消会话。
- 系统提示词预览、附件解析。
...
- 用户管理：`/wunder/admin/user_accounts`、`/wunder/admin/user_accounts/test/seed`、`/wunder/admin/user_accounts/test/cleanup`、`/wunder/admin/user_accounts/{user_id}`、`/wunder/admin/user_accounts/{user_id}/password`、`/wunder/admin/user_accounts/{user_id}/tool_access`、`/wunder/admin/user_accounts/{user_id}/agent_access`。
...
- `frontend` 维持原 UI，仅调整接口与端口：
  - API 基址：运行时优先读取 `/config.json` 的 `api_base`，为空时回退到 `VITE_API_BASE`/`VITE_API_BASE_URL`，再回退 `http://localhost:18000/wunder`。
  - Vite dev 端口：18001。
  - Nginx 部署可通过挂载 `frontend/public/config.json` 覆盖 `/config.json`（容器内映射到 `/etc/wunder/config.json`），无需重新构建前端。
...
- 新增 `web/modules/org-units.js` 处理单位结构维护与负责人配置。
...
## 2026-02-09 Frontend TypeScript Migration
- All business scripts under `frontend/src` are now migrated to TypeScript (`.ts` + `<script setup lang="ts">`).
- Added TypeScript project baseline: `tsconfig.json`, `tsconfig.node.json`, `src/env.d.ts`, `src/shims-vue.d.ts`.
- Build config moved to `vite.config.ts`; added `npm run typecheck` and `npm run build:check` as quality gates.
- API paths and transport behavior remain unchanged (WebSocket first, SSE fallback).
