...
flowchart LR
  ...
  class LLM,ExtDB,KB,VectorDB external;
  class WS volume;
...
- 服务层：`src/services/`，包含 LLM/MCP/知识库/向量知识库/Skills/工具/历史/工作区等能力封装。
...
- 向量知识库：`base_type=vector` 通过嵌入模型与 Weaviate 检索，文档切片与索引由管理端/用户端接口维护。
...
- 会话锁按 `user_id + agent_id` 维度限制并发，支持同用户多智能体并行；chat_sessions 记录 `agent_id` 与 `tool_overrides` 用于恢复上下文与工具集。
...
- 智能体配置存储于 `user_agents`，包含名称、描述、系统提示词、工具集、共享与图标信息。
- 用户侧通过 `/wunder/agents`、`/wunder/agents/shared` 获取卡片数据，`/wunder/agents/running` 汇总运行中智能体。
- 访问控制基于 `user_agent_access` 维护允许/禁用列表，确保共享智能体按策略可见。

### 5.7 多智能体并行与会话隔离

- Orchestrator 使用 `session_locks(user_id, agent_id)` 限制同一智能体串行执行。
- 工作区通过 `scoped_user_id` 生成 `user__a__hash` 目录，实现同用户多智能体隔离。
- 会话恢复按 `agent_id` 维度归档，支持按智能体继续对话与工具设置继承。

### 5.8 向量知识库

- 向量知识库文档落盘在 `vector_knowledge/shared` 或 `vector_knowledge/users/<user_id>`，元数据与原文分离存储。
...
| `knowledge/` | 本地知识库目录 |
| `vector_knowledge/` | 向量知识库存储（shared/users/<user_id>，文档与切片元数据） |
...
- 新知识库：通过 MCP、本地 `knowledge/` 或 `vector_knowledge/` + Weaviate 扩展检索能力。
