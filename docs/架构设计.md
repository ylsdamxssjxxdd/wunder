...
flowchart LR
  ...
  subgraph Runtime["运行服务"]
    ...
    RustAPI["核心<br/>wunder_engine&nbsp;&nbsp;"]
    Sandbox["沙盒<br/>sandbox"]
    MCP["MCP服务<br/>wunder_mcp"]
    PG[(数据库<br/>postgres)]
...
| 服务 | 角色 | 说明 |
| --- | --- | --- |
| wunder_engine | 核心 API 服务 | Rust + Axum，对外暴露 `/wunder`、`/a2a` 与管理端接口；同时挂载管理端静态页面。 |
...
| wunder_mcpntend | 构建任务 | 只负责 `npm install && npm run build`，构建后退出。 |
...
- API 接入层：统一鉴权、参数校验与路由，输出 WebSocket/SSE 与非流式响应。
...
- **主链路**：用户（user_id）→ 智能体应用（agent_id） → 会话/智能体线程（session_id） → 智能体循环（agent_loop）→ 流式事件（WS/SSE）
