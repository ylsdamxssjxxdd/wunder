...
- `/wunder/temp_dir/*` 默认落在本地 `./temp_dir`（容器内 `/app/temp_dir`；可用 `WUNDER_TEMP_DIR_ROOT` 覆盖）。
- 运行态可写配置保留在仓库本地 `data/`（`data/config`、`data/prompt_templates`、`data/user_tools` 等），便于直观治理与迁移。
- 构建与依赖缓存（`target/`、`.cargo/`、根 `node_modules/`）保持写入仓库目录（bind mount），便于本地清理与管理。
...
Desktop 同样使用一个 temp_root（默认 `<app_dir>/WUNDER_TEMPD`，Electron/AppImage 通过 `--temp-root` 指向 `userData/WUNDER_TEMPD`），并额外有一份桌面端 settings。

固定/随包配置：

- `desktop/tauri/tauri.conf.json`：桌面端打包/能力声明（构建期配置）
...
- `WUNDER_TEMPD/config/wunder.override.yaml`：Desktop 运行时覆盖配置（与 server 同机制）
  - 首次启动若该文件不存在，会尝试从以下来源自动播种：`WUNDER_DESKTOP_PRECONFIG_PATH`、AppImage/程序同目录的 `预配置文件.yml|yaml` / `wunder.override.yml|yaml`、以及打包资源内置的 desktop 预配置模板
  - 若 `desktop.settings.json` 已保存 `llm`，则该本地设置会继续覆盖 override 中的模型配置
...
- 默认 `<app_dir>/WUNDER_WORK`，Electron/AppImage 通过 `--workspace` 指向 `userData/WUNDER_WORK`；也可在 `desktop.settings.json` 或启动参数中指定
...
- `docker-compose-x86.yml` / `docker-compose-arm.yml` / `packaging/docker/docker-compose-ubuntu20.yml`：容器化部署编排参数（含 `WUNDER_CONFIG_PATH` / `WUNDER_CONFIG_OVERRIDE_PATH`）
- `Dockerfile`：镜像构建相关参数
...
- `config/benchmark/tasks/*.md`：PinchBench 风格能力评估任务集（Markdown + YAML frontmatter）
...
- `extra_mcp/mcp_config.json`：MCP 服务自身的外部依赖配置（数据库/RAG 等）
...
- `benchmark_runs` / `benchmark_attempts` / `benchmark_task_aggregates`：Benchmark 运行、attempt 与任务聚合结果
...
3) 把密钥类配置（如 `extra_mcp/mcp_config.json` 里的 key/password）迁移到环境变量或独立密钥管理。
...
- `extra_mcp/mcp_config.json`：目前含明文口令/密钥且与主系统治理脱节，建议至少做到：
  - 配置项迁到环境变量/密钥管理；或
  - 由 wunder-server 作为“配置中心”提供读取接口，extra_mcp 启动时拉取（并缓存）。
...
- 本地目录（bind mount）：`./temp_dir/**`（临时文件区；可用 `WUNDER_TEMP_DIR_ROOT` 覆盖）
- 命名卷 `wunder_workspaces`：挂载到 `/workspaces`（用户工作区）
...
构建/依赖缓存（`target/`、`.cargo/`、根 `node_modules/`）默认在仓库目录（bind mount）中，便于本地清理与管理。
