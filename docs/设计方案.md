...
- **忙时排队**：主线程忙碌时请求进入 `agent_tasks`，空闲后由 `AgentRuntime` 依序执行。
...
- 并发与会话互斥先使用进程内锁实现，后续可接入数据库 `session_locks` 以支持跨进程互斥。
...
- 当主线程忙碌时，请求进入 `agent_tasks` 队列；队列任务由 `AgentRuntime` 异步执行并通过 `stream_events` 回放。
