...
- 新增 `/wunder/gateway/ws` 作为多端控制面入口，角色统一为 `operator`/`node`/`channel`。
- Gateway 连接使用 `connect` 进行协议协商与角色声明，服务端返回 `hello-ok` + presence 快照。
- 节点能力以 `node.invoke` 工具对外暴露，LLM 可按 `node_id` 触发节点执行。
- 管理端提供 `/wunder/admin/gateway/*` 用于节点注册、token 管理与在线状态查询。
