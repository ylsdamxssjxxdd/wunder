...
- 提示词模板按职能拆分为角色、安全、产品、编程、运行环境、工具协议、工程师信息等模块，统一由 Prompt Builder 组装；`function_call` 模式不注入工具协议与工具清单，但仍保留技能提示词。
...
- 提示词模板优先读取 `prompts/zh|en` 文件（可通过 `WUNDER_PROMPTS_ROOT` 指定根目录），文件缺失时回退二进制内嵌模板。
