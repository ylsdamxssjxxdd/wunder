...
- 新增 `wunder-mcp` 服务用于运行 `mcp_server/` 下的 FastMCP 脚本，默认以 streamable-http 暴露 MCP 入口，人员数据库连接通过 `PERSONNEL_DB_*` 配置。
- MCP 服务支持 `mcp_server/mcp_config.json` 统一配置（`MCP_CONFIG_PATH` 可指定路径），环境变量优先级更高。
- 人员数据库 MCP 支持 MySQL/PostgreSQL，多库配置通过 `PERSONNEL_DB_TARGETS`/`PERSONNEL_DB_TARGETS_PATH`，工具入参用 `db_key` 选择目标；单库可用 `PERSONNEL_DB_TYPE` 切换类型。
...
- `user_agents`：用户智能体配置（agent_id/name/system_prompt/tool_names/access_level）
- `user_agent_access`：用户智能体权限覆盖（allowed_agent_ids/blocked_agent_ids）
- `user_tool_access`：用户工具权限覆盖（allowed_tools/blocked_tools）
...
- 智能体提示词作为追加层注入（基础系统提示词 → 用户 extra_prompt → 智能体 system_prompt → 会话覆盖）。
...
- 功能广场新增“工具管理”与“智能体广场”，聊天页支持会话级工具调整与智能体入口跳转。
