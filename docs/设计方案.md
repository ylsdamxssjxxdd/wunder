...
- docker compose 默认使用两个命名卷：`wunder_workspaces` 挂载到 `/workspaces`（用户工作区与临时目录，`WUNDER_TEMP_DIR_ROOT` 默认 `/workspaces/temp_dir`），`wunder_logs` 挂载到 PostgreSQL/Weaviate 数据目录（`/var/lib/postgresql/data`、`/var/lib/weaviate`）；其余运行态可写配置保持在仓库本地 `data/`，避免过度卷化；构建/依赖缓存（`target/`、`.cargo/`、`frontend/node_modules/`）保持写入仓库目录便于管理。
