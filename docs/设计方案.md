...
- 新增 `wunder_mcp` 服务用于运行 `mcp_server/` 下的 FastMCP 脚本，默认以 streamable-http 暴露 MCP 入口，人员数据库连接通过 `mcp_server/mcp_config.json` 的 `database` 配置。
...
- 新增 `/wunder/channel/feishu/webhook` 与 `/wunder/channel/qqbot/webhook`，统一进入 ChannelHub 主链路并支持账号级直连出站。
...
- 渠道账号扩展配置：`feishu.*` 与 `qqbot.*`（app_id/secret/签名参数等）直接挂载在 `channel_accounts.config`。
...
- 用户侧前端位于 `frontend/`，开发端口默认 18001（Vite dev server），生产静态资源由 `wunder_nginx` 提供，默认 18002。
...
为保证方案文档与当前代码一致，补充以下基线说明。

### 11.1 统一二进制双模式

- wunder-server 同时承担 api 模式和 sandbox 模式。
- api 模式承载调度主链路、管理端接口、渠道接入、网关控制面和内置 mcp 路由。
- sandbox 模式独立暴露沙盒执行接口。

### 11.2 三类入口并行

- 用户请求入口。wunder 与 chat 接口族，支持 ws 和 sse。
- 渠道消息入口。whatsapp、feishu、qq 及通用 provider webhook。
- 控制面入口。gateway ws，支持节点 presence 与远程调用。

### 11.3 数据模型扩展

当前已形成会话域、调度域、渠道域、网关域、治理域、评估域和向量文档域的完整关系模型。详细字段与索引见数据库设计文档。

### 11.4 可观测与治理闭环

- MonitorState 负责会话状态机、事件留存和回放。
- Throughput、Performance、Evaluation 形成压测与质量回归工具链。
- 用户和组织治理通过 user_accounts、org_units、access 相关表落地。

### 11.5 结构演进原则

- 新能力优先做成工具或服务模块接入 Orchestrator。
- 高风险能力优先走 sandbox 隔离。
- 数据层保持应用层关系约束与自动补齐迁移策略，确保原型迭代速度。


## 12. 蜂巢与蜂群重构落地（2026-02）

- 将智能体分组主键从 `sandbox_container_id` 迁移为 `hive_id`，容器编号仅保留为执行资源位概念。
- 新增 `hives` 资源，并在 `user_agents` 中以 `hive_id` 建立归属关系，支持按蜂巢隔离与批量管理。
- 新增 `team_runs/team_tasks` 数据模型与接口，沉淀蜂群运行轨迹、任务状态、汇总结果与时延指标。
- 扩展内置工具 `agent_swarm`，提供 `list/status/send/history/spawn` 行为，并统一蜂巢作用域校验。
- 事件流补充 `team_*` 系列事件，前端可基于会话维度实时展示蜂群任务分发、执行与收敛过程。
- 前端门户支持蜂巢切换/新建（可复制已有蜂巢）与“新建应用复制已有应用”，降低蜂群构建成本。

## 13. 单蜂巢简化（2026-02）

- 系统回归为单蜂巢模型，`hive_id` 固定为 `default`，不再支持用户创建、切换或复制多个蜂巢。
- 后端移除 `/wunder/hives` 相关路由，智能体与蜂群运行统一归属默认蜂巢。
- `agent_swarm`、`team_runs`、`user_agents` 中与多蜂巢隔离相关的校验与筛选逻辑已下线，避免跨蜂巢分流带来的复杂度。
- 前端门户移除蜂巢切换与创建交互，保留单组智能体应用管理体验。

## 2026-02-09 Frontend Engineering Update (TS)
- User frontend (`frontend`) completed full migration from JS to TS.
- Frontend validation gate is now `vue-tsc --noEmit` + `vite build`.
- No backend API protocol change is introduced by this migration.

## wunder-cli（本地单用户运行形态）

- 新增根目录 `wunder-cli/` 模块，提供本地命令行能力（无子命令默认进入 TUI，支持 ask/chat/resume/tool/exec/mcp/skills/config/doctor）。
- 运行时状态统一写入启动目录 `WUNDER_TEMP/`，包括 SQLite、override 配置、会话文件、用户工具配置。
- 默认读取 `config/wunder.yaml`；若 CLI 无法访问仓库配置，将自动生成 `WUNDER_TEMP/config/wunder.base.yaml` 作为基础配置并继续启动。
- 通过 `AppStateInitOptions::cli_default()` 构建轻量状态，默认关闭 team runner / agent runtime / cron / gateway maintenance。
- 通过环境覆盖复用核心资源路径（prompts、i18n、user_tools、vector_knowledge、skill_runner）。
- 单根工作区模式下，智能体工作目录即 CLI 启动目录。
- CLI 支持 `tool_call/function_call` 模式切换（请求级临时覆盖 + override 持久化）。
- MCP 管理命令支持 `list/get/add/remove/enable/disable`，并提供文本与 JSON 输出。
- 提示词模板改为二进制内嵌（内置模板），CLI 离线单文件部署仍可运行；`WUNDER_PROMPTS_ROOT` 只用于可选外部覆盖。
- 交互态 slash 命令已覆盖 `/help`、`/status`、`/model`、`/tool-call-mode`（`/mode`）、`/session`、`/system` 与 `/config` 系列。
- TUI 采用 `tui/mod.rs + tui/app.rs + tui/ui.rs` 分层组织（生命周期/状态机/渲染），便于治理与后续扩展。
- `/config` 在 TUI 与行式 `chat` 都支持向导流程（`base_url -> api_key -> model -> max_context`），也支持 `/config <base_url> <api_key> <model> [max_context]` 一行参数；`/model` 与 `/tool-call-mode` 继续作为会话内快速切换。
- TUI 状态栏聚焦 `context left`，会话区支持 PgUp/PgDn/Home/End 与鼠标滚轮滚动。
- TUI 多行编辑体验已对齐 codex 快捷键：`Shift+Enter`/`Ctrl+J` 换行，`Ctrl+B/F` 字符移动，`Alt+B/F/Left/Right` 按词导航，`Ctrl+W`/`Alt+Backspace`/`Alt+Delete` 按词删除。
- 输入区会根据视口宽度自动折行，并支持按键 Repeat 事件下的连续光标移动。
- 鼠标模式支持 `F2` 或 `/mouse [scroll|select]` 切换：`scroll` 用于滚轮浏览，`select` 用于框选复制。
- 流式偏移读取修复为空会话返回 `0`，`LLM unavailable` 错误会携带 detail 字段。

## wunder-desktop（本地桌面运行形态，M0/M4 已落地）

- 新增 `wunder-desktop` 二进制入口（`wunder-desktop/main.rs`），默认走 **Tauri 桌面窗口模式**，双击程序即可打开 GUI；保留 `--bridge-only` 诊断模式。
- release 构建启用 `windows_subsystem = "windows"`，Windows 下双击不会弹出终端窗口。
- 启动时自动创建运行目录：`<app_dir>/WUNDER_TEMPD`，SQLite 固定为 `WUNDER_TEMPD/wunder_desktop.sqlite3`，并持久化 `WUNDER_TEMPD/config/desktop.settings.json`。
- 默认工作目录为 `<app_dir>/WUNDER_WORK`，支持 `--workspace` 覆盖并持久化；工作目录、MCP、Skills、内置工具链均复用 `src/` 核心。
- 通过 `AppStateInitOptions::desktop_default()` 走轻量初始化，默认关闭 channels/gateway/agent_queue/cron，优先保证本地资源占用稳定。
- desktop 路由聚合函数 `build_desktop_router` 已落地，暴露普通用户链路：`auth/chat/chat_ws/core/core_ws/external_links/workspace/user_tools/user_agents/user_channels/mcp/temp_dir`。
- 桌面前端回归用户侧布局（`UserLayout` + 既有 `UserTopbar`），不再使用桌面专属侧边栏；聊天/世界/工具/设置/个人页均复用用户侧页面，保持交互一致。
- 启动注入 `window.__WUNDER_DESKTOP_RUNTIME__`，并自动写入 `localStorage.access_token`；desktop 模式路由默认跳转 `/desktop/home`，无需登录流程。
- desktop 新增“容器管理 / 系统设置”两类高级页面：容器管理用于维护 container_id -> 工作目录映射（容器 1 默认 WUNDER_WORK）；系统设置用于维护语言、LLM 模型配置与远端 wunder-server 连接参数。
- 桌面设置通过 GET/PUT /wunder/desktop/settings 读写 WUNDER_TEMPD/config/desktop.settings.json，并同步更新运行中的 ConfigStore 与 WorkspaceManager 映射。
- 当 remote_gateway 启用且地址可解析时，desktop 会将 runtime 的 `api_base/ws_base` 切到远端；不自动登录或自动注册，用户按正常 auth 流程完成注册/登录。
- runtime 同时暴露 `desktop_token`（本地设置接口鉴权）与远端模式元信息；远端 `access_token` 由用户登录后写入，确保“远端业务 + 本地设置”可并存。
- 本地桥接接口新增：
  - `GET /config.json`：提供 `api_base/ws_base/token/desktop_token/user_id/workspace_root/mode` 运行时配置（含 remote 状态字段）。
  - `GET /wunder/desktop/bootstrap`：提供完整 desktop 运行时信息（含路径与 token）。
- Tauri 侧提供命令 `desktop_runtime_info`，供前端读取当前 runtime 快照。
- Desktop 工程资源统一收敛到 wunder-desktop/：build.rs、tauri.conf.json、capabilities/、icons/ 均位于该目录，根目录不再散落 Tauri 资产。
- API 鉴权由 desktop token guard 统一控制：支持 `x-api-key`、`Authorization: Bearer`、`sec-websocket-protocol`、`access_token/api_key` 查询参数。
- 通信策略保持“WebSocket 优先、SSE 兜底”，直接复用 `/wunder/chat/ws` 与 chat stream/resume 事件协议。
