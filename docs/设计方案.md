...
- 新增根目录 `wunder-cli/` 模块，提供本地命令行能力（默认交互，支持 ask/chat/resume/tool/exec/mcp/skills/config/doctor）。
...
- `/config` 支持“地址 -> API Key -> 模型名”引导式配置，`/model` 与 `/tool-call-mode` 支持会话内快速调整。
