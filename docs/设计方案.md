...
- 新增根目录 `wunder-cli/` 模块，无子命令默认进入 TUI，并支持 `ask/chat/resume/tool/exec/mcp/skills/config/doctor`。
...
- CLI 支持 `tool_call/function_call` 模式切换、`resume` 会话恢复、JSONL 事件输出与 codex 风格 TUI。
