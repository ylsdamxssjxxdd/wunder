...
- 详细协议与节点说明参见：`docs/WebSocket-Transport.md`。
...
- 新增飞书长连接后台 worker：扫描 active 且存在用户侧绑定的飞书账号并调用 `/callback/ws/endpoint` 建立 WebSocket，接收事件后统一进入 ChannelHub 处理。
- `/wunder/admin/channels/accounts` 返回账号级 `runtime` 信息（含飞书长连接状态与绑定计数），供管理端快速排查接入状态。
...
- 新增用户侧 `/wunder/channels/*` 接口与前端“渠道”页面，允许用户自助绑定 channel/account/peer 到指定智能体应用。
