...
- 共享沙盒服务仍可复用现有 Python 实现，通过 `WUNDER_SANDBOX_ENDPOINT` 连接；运行时会优先读取该环境变量，并在常见的 `sandbox`/`127.0.0.1` 地址之间自动回退，减少内网迁移时的 IP 配置问题。
- Rust 版已在内置工具“执行命令/ptc”接入共享沙盒服务：`sandbox.mode=sandbox` 时调用 `/sandboxes/execute_tool`，并在工具结果中标记 `sandbox=true`。
...
- 用户侧鉴权与会话接口：`/wunder/auth/*` 与 `/wunder/chat/*`；管理员通过 `/wunder/admin/user_accounts/*` 管理账号、重置密码与工具权限。
...
- 管理端系统设置支持动态调整 `server.max_active_sessions` 与 `sandbox.mode`，写入覆盖配置并即时生效。
- 并发与单用户互斥先使用进程内锁实现，后续可接入数据库 `session_locks` 以支持跨进程互斥。
...
- 工具调用轮次会把上一轮 assistant 的原始输出（`tool_call` 模式含工具调用标签）与 reasoning 写入下一轮 messages；`function_call` 模式回填 assistant 的 `tool_calls` 与 role=tool/tool_call_id 结果，保持上下文完整。
...
- 最终回复事件补充 `stop_reason`（`model_response/final_tool/a2ui/max_rounds/unknown`），便于调试面板定位停止原因。
...
- 内置工具由 `src/tools.rs` 统一维护规格与执行入口，确保配置、提示词与执行一致。
...
- 个人知识库上传当前仅接受 Markdown，非 Markdown 文件需先调用 `/wunder/doc2md/convert` 转换。
...
- 按 `user_id` 创建持久化工作区：
- `workspaces/{user_id}/`：用户可见文件区（提示词中使用 `/workspaces/{user_id}/` 作为工作目录）
- 若用户 24 小时内无会话，自动清空该用户临时文件区内容。
...
- 提示词模板拆分为系统、工具协议与工程师信息块，统一由 Prompt Builder 组装；`function_call` 模式不注入工具协议与工具清单，但仍保留技能提示词与用户附加提示词。
- 提示词文件集中在 `app/prompts`，`tool_call` 模式按需注入工具清单与环境信息，`function_call` 模式保持简化。
...
- 字面知识库支持上传多格式文件，前端可先调用 `/wunder/doc2md/convert` 转为 Markdown 后写入知识库目录。
...
- 账号状态/角色/访问级别由管理员在 `/wunder/admin/user_accounts/*` 维护，线程 `user_id` 与注册用户仍保持区分。
...
## 7. 配置文件规范（基础 + 覆盖）
  ...
  i18n 文案默认读取 `config/i18n.messages.json`（可通过 `WUNDER_I18N_MESSAGES_PATH` 指定），缺失时回退 `app/core/i18n.py`。
...
- 统一错误码与异常封装，前端易于处理。
- 日志分级：请求日志、工具日志、模型日志。
- 并发控制：同一 user_id 互斥 + `server.max_active_sessions` 上限，session_locks TTL 心跳保障跨进程互斥。
- 指标缓存：系统快照/日志占用/工作区占用与工作区树/搜索按 TTL 缓存，降低频繁 IO。
