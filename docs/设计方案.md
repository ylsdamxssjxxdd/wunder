...
- 新增 `/wunder/channel/feishu/webhook`、`/wunder/channel/wechat/webhook` 与 `/wunder/channel/qqbot/webhook`，统一进入 ChannelHub 主链路并支持账号级直连出站。
...
- 渠道账号扩展配置：`feishu.*`、`wechat.*` 与 `qqbot.*`（app_id/secret/签名参数等）直接挂载在 `channel_accounts.config`。
