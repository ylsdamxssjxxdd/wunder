...
- 工具调用轮次会把上一轮 assistant 的原始输出（含 tool_call）与 reasoning 写入下一轮 messages，保持上下文完整。
...
- 工具规格与可用性汇总由 Rust 工具清单构建逻辑生成，`/wunder/tools` 与提示词注入共享同一策略。
...
- MCP 工具：基于 rmcp 客户端连接服务端，由管理员在配置中选择性启用后注入系统提示词，调用形式为 server@tool。
