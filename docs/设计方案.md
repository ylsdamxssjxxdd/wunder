...
- ????? `wunder-cli/` ???????????????????? ask/chat/resume/tool/exec/mcp/skills/config/doctor??
- ????????????? `WUNDER_TEMP/`??? SQLite?override ???????????????
- ?? `AppStateInitOptions::cli_default()` ??????????? team runner / agent runtime / cron / gateway maintenance?
- ???????????????prompts?i18n?user_tools?vector_knowledge?skill_runner??
- ????????????????? CLI ?????
- CLI ?? `tool_call/function_call` ???????????? + override ?????
- MCP ?????? `list/get/add/remove/enable/disable`??????? JSON ???
