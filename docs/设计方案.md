...
- desktop 新增“容器管理 / 系统设置”两类高级页面：容器管理用于维护 container_id -> 工作目录映射（容器 1 默认 WUNDER_WORK）；系统设置用于维护语言、LLM 模型配置与“连接 wunder-server 注册角色”预留参数。
...
- 本地桥接接口新增：
  - `GET /config.json`：提供 `api_base/ws_base/token/user_id/workspace_root/mode` 运行时配置。
