...
- 提供 `Dockerfile.rust` 与 `docker-composex86.yml` / `docker-composearm.yml`，用于 x86/arm64 的运行与热重载环境。
...
- 用户侧优先扩展 `/wunder/chat/ws`，后续可补齐 `/wunder/ws` 统一入口版本。
...
- 内置工具（EVA 风格）：最终回复、a2ui、计划面板、问询面板、a2a观察/a2a等待、执行命令、ptc、列出文件、搜索内容、读取文件、技能调用、写入文件、替换文本、编辑文件、子智能体控制（列表/历史/发送/派生）。
