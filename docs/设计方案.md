...
- 交互态增加 codex 风格 slash 命令路由：`/help`、`/status`、`/model`、`/tool-call-mode`（`/mode`）与 `/config` 系列。
- TUI 采用 `tui/mod.rs + tui/app.rs + tui/ui.rs` 分层组织（生命周期/状态机/渲染），便于治理与后续扩展。
- `/config` 在 TUI 支持单行参数配置（`/config <base_url> <api_key> <model>`），`chat` 行式模式保留“地址 -> API Key -> 模型名”引导式配置；`/model` 与 `/tool-call-mode` 支持会话内快速调整。
- 流式偏移读取修复为空会话返回 `0`，`LLM unavailable` 错误会携带 detail 字段。

## wunder-desktop（本地桌面运行形态，M0/M1）

- 新增 `wunder-desktop` 二进制入口（`wunder-desktop/main.rs`），面向本地普通用户场景。
- 启动时自动创建运行目录：`<app_dir>/WUNDER_TEMPD`，并将 SQLite 固定到 `WUNDER_TEMPD/wunder_desktop.sqlite3`。
- 默认工作目录为 `<app_dir>/WUNDER_WORK`，支持通过参数覆盖并写回 `WUNDER_TEMPD/config/desktop.settings.json`。
- 通过 `AppStateInitOptions::desktop_default()` 走轻量初始化，默认关闭 channels/gateway/agent_queue/cron。
- 新增 desktop 路由聚合函数 `build_desktop_router`，仅暴露普通用户链路，避免管理面路由默认暴露。
- 本地通信桥按“WebSocket 优先、SSE 兜底”沿用原有 `/wunder/chat/ws` 与 chat stream 事件模型。
- Desktop 启动时生成并持久化本地 token，默认写入配置并用于 API 访问保护。
