...
- 管理端新增吞吐量压测接口，服务端从并发 1 按步增压测到最大并发（步增为 0 时仅压测最大并发），采样总/单预填充与总/单解码速度等指标，支持多问题随机发送、结果保存/导出与历史回放。
...
- 根路径 `/` 提供简易聊天测试页，直接调用模型端点（OpenAI 兼容 `chat/completions`），支持填写模型端点/API Key/模型名、图片与文本附件上传与思考折叠显示，并在浏览器中保存设置与对话历史。
...
- 调试面板支持上传文件/图片，文件通过内置 Rust doc2md 转换接口（`/wunder/doc2md/convert`）解析后附加请求，图片按多模态格式发送。
