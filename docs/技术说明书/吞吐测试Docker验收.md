...
- 内置虚拟模型：按当前消息估算输入 Token，快 / 中 / 慢档预填充分别为 2000 / 500 / 100 Token/s，思考和生成分别为 200 / 50 / 10 Token/s，默认快；使用实际时钟和共享流式指标解析器。先思考（目标输出的 1/4）再生成内容，两者合计恰好等于指定输出数量。吞吐模拟不读取虚拟回放日志，不创建线程。
- HTTP 夹具：通过 Chat Completions SSE 返回思考与生成内容流，使用快档速度。预填充期间发送角色帧和心跳，验证这些事件不被误认为首个输出 Token。
- 另设提前结束、缺失 usage、断流、拒绝参数四种夹具模式，验证未达标与失败不会伪装为成功。
- 输入档位是估算，usage 与首字耗时用于算实测输入速度；网络和定时器开销使结果略低于配置值。并发详情同时显示平均/最慢首字延迟、整体批次生成/预填充速度和单请求平均速度；生成速度排除首字等待，思考作为输出的子项单列，超过 1000ms 的延迟以秒显示。
