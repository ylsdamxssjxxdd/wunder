...
- Self-hosted MCP: `/wunder/mcp`, visible but disabled by default; enables `wunder@run`.
...
- Knowledge upload supports multi-format conversion with doc2md + Python fallback.
...
- Static assets served at `/wunder/web`.
...
- `observability.monitor_event_limit` <= 0 means no truncation.
...
## 11. Iteration plan (suggested)

- MVP: single model + built-in tools + `/wunder` SSE.
- M1: MCP + Skills + persistent workspace.
- M2: multi-model routing, rate limit, monitoring.

## 12. Key risks & mitigations

- Unstable tasks: retries and fallbacks.
- Tool misuse: allowlists and permission tiers.
- Concurrency pressure: async IO + queues + limits.
