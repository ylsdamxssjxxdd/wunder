...
自托管 MCP 服务 `/wunder/mcp` 已由 Rust 版实现，基于 streamable-http 提供工具入口（wunder@excute、wunder@doc2md）。
线程 `user_id` 作为会话标识与隔离维度，可使用任意虚构名称，不要求已注册；注册用户仅用于用户端登录与权限管理。
注册用户按访问级别分配每日请求额度（A=10000/B=1000/C=100），0 点重置；虚拟 user_id 不受额度限制。
额度按“每次模型调用”消耗，单次请求若多轮调用会多次计入；超额返回 429 并提示额度耗尽，SSE 会同步 `quota_usage` 事件供前端展示消耗明细。
管理员可在用户管理中调整每日额度，前后端统一提示额度剩余与超额原因，便于可视化成本控制与业务协作。
当前默认实现为 Rust，Python app 已移除；`wunder-server` 可通过 `WUNDER_SERVER_MODE=sandbox` 以沙盒模式启动，配合独立容器提供共享沙盒服务；`Dockerfile.rust` + `docker-compose.rust.x86.yml`/`docker-compose.rust.arm.yml` 用于运行与热重载。
docker compose 默认使用命名卷 `wunder_postgres` 存放 PostgreSQL 数据。
...
为解决 ptc/Matplotlib 与其他字体依赖的中文缺字问题，docker compose 会把仓库 `fonts/` 挂载到容器 `/usr/share/fonts/wunder`，同时提供 `config/fonts.conf` 与 `config/matplotlibrc`，字体缓存分别写入 `/workspaces/.fontconfig` 与 `/workspaces/.matplotlib`。

它的核心价值是：
- 统一入口 + 强工具编排：把多模型、多工具、多技能整合到一个调度面。
- 多用户隔离 + 持久化工作区：支持多人并发与长期对话资产沉淀。
- 可观测 + 可管控：线程级监控、工具调用热力图、可取消任务。
- security.exec_policy_mode（allow/audit/enforce）用于高风险命令审计/拦截，并支持会话级审批缓存。
- `context_usage` 事件回传上下文占用，监控展示真实使用量。
- 额度治理 + 成本可控：按访问级别分层配额、每次模型调用精确计数、管理员可动态调节并支持前端实时提示。
- 多供应商预设：LLM 配置内置 OpenAI 兼容 provider 预设（如 openai/openrouter/siliconflow/deepseek/moonshot/qwen 等），便于快速接入。
- 吞吐量测试 + 性能采样 + 能力评估：按并发列表压测量化总/单预填充与总/单解码速度，性能采样观察系统能力基线，用例集评分支持回归对比。
- 长期记忆：按用户开启自动总结并注入系统提示词，以 `[长期记忆]` 标签+纯文本段落强化长期上下文，每条记忆附带“年月日时分”时间前缀。
- 语言可切换：前端设置语言后，系统提示词与接口返回消息同步切换中英文，翻译默认来自 `config/i18n.messages.json`（可用 `WUNDER_I18N_MESSAGES_PATH` 覆盖），支持语言以 `i18n.supported_languages` 配置为准。
- LSP 代码理解：支持按项目启动语言服务器，为模型提供定义/引用/符号/诊断能力，并可在管理端配置与测试。
...
- Orchestrator（`src/orchestrator/`）：任务编排、工具选择与 SSE 事件流输出。
- 工具层（`src/services/tools.rs`）：内置工具规格与执行入口。
- LSP 管理器（`src/lsp/mod.rs`）：按 user_id + 项目根目录管理 LSP 进程，提供诊断与代码导航能力。
- 工作区层（`src/services/workspace.rs`）：用户工作区文件管理与压缩下载。
- 监控层（`src/ops/monitor.rs`）：线程状态与系统资源快照，按配置持久化到存储后端。
- 吞吐量压测（`src/ops/throughput.rs`）：服务端按并发列表逐档压测，采样总/单预填充与总/单解码速度等指标，使用内置题库随机发送问题，结果可保存/导出并支持历史回放。
- 性能测试（`src/ops/performance.rs`）：按并发采样系统提示词构建、文件操作、命令执行、权限/工具解析与日志写入耗时。
- 能力评估（`src/ops/evaluation_runner.rs` + `src/api/evaluation.rs`）：按用例集跑评估，启动时清理该用户 eval 临时目录并预生成明细为 `active`，提供评分与历史回溯/删除；用例文件存放在 `config/evaluation/cases`。
- 配置层（`src/core/config.rs` + `src/core/config_store.rs`）：基础配置与管理端覆盖写入，支持通过管理端调整 `server.max_active_sessions`、`server.stream_chunk_size`、`security`、`sandbox`、`observability`、`cors` 等配置并持久化。
- 沙盒服务（`src/sandbox/server.rs`）：独立容器运行 `WUNDER_SERVER_MODE=sandbox`，提供 `/sandboxes/execute_tool` 与 `/sandboxes/release`。
...
补充：每次调用模型前会校验并扣减注册用户的当日额度，超额立即返回并提示；虚拟 user_id 不受限。
补充：当最终回复完成后，系统会将该会话加入长期记忆总结队列，按完成时间顺序逐条生成记忆摘要并落库；总结请求使用 `prompts/memory_summary.txt` 作为系统提示词，并将历史消息融合为单条用户内容，同时写入任务日志供历史队列追踪。
...
| --- | --- | --- | --- | --- |
| 内置工具 | `src/services/tools.rs` | 直接调用工具名 | 管理员配置启用 | 文件操作、命令执行、ptc、计划面板、问询面板 |
| MCP 工具 | `config/wunder.yaml` 基础配置 + `data/config/wunder.override.yaml` 覆盖 + `/wunder/admin/mcp` | 形如 `server@tool` | 管理员启用 + allow_tools | 连接外部服务/平台能力（含自托管 `wunder@excute`/`wunder@doc2md`） |
...
### 4.2 工具调用机制
- 工具调用支持两种模式：`tool_call`（默认，EVA 风格 `<tool_call>{"name":"工具名","arguments":{...}}</tool_call>`）与 `function_call`（OpenAI 风格 `tool_calls/function_call`，工具结果以 role=tool + tool_call_id 回传），由模型配置 `tool_call_mode` 控制。
- 会话轮次区分用户轮次与模型轮次：用户每次发送消息记 1 轮用户轮次；模型每执行一次动作（模型调用、工具调用或最终回复）记 1 轮模型轮次；事件数据以 `user_round/model_round` 标识。
- 模型轮次中的工具调用会将上一轮 assistant 原始输出（`tool_call` 模式含 `<tool_call>` 标签）与 reasoning 追加进下一轮模型请求；`function_call` 模式则回填 assistant 的 `tool_calls` 与 role=tool/tool_call_id 结果，保证上下文可追溯。
- 工具注入策略：`tool_call` 模式下仅将 `tool_names` 选择的工具注入系统提示词；`function_call` 模式不注入工具提示词，工具清单仅通过请求 `tools` 传入。
- 会话系统提示词在首次构建后固定，用于历史还原；后续工具勾选或工作区变化不会刷新提示词，执行仍按最新可用工具集为准。
- 内置工具支持英文别名（如 `read_file`、`write_file`），模型与接口可使用英文名触发调用。
- `a2ui` 为可选输出工具，显式勾选后会替换“最终回复”工具，用于输出 A2UI 消息并在调试面板渲染 UI。
- 新增 `计划面板` 工具，用于更新任务计划并触发 `plan_update` 事件，前端自动弹出计划看板。
- 新增 `问询面板` 工具，用于给出多条路线供用户选择并触发 `question_panel` 事件，用户可选路线或直接继续对话。
- A2A 服务工具由 `/wunder/admin/a2a` 配置并缓存 AgentCard，启用后以 `a2a@service` 形式对模型可用；`tool_call` 模式会注入提示词，`function_call` 模式通过 tools 协议传入；支持 `internal/external` 区分，其中 `internal` 表示 Wunder 内部 A2A 服务并需预置 `user_id` 以自动填充。
...
- Skills 在两种模式下都会注入已启用的技能元信息，确保模型了解技能用法。
- 知识库按配置生成独立工具，`tool_call` 模式下未启用项不会注入工具清单（`function_call` 仅通过 tools 协议传入）。
- 自托管 MCP 工具 `wunder@excute` 在内部执行时会剔除自身工具，并固定 `user_id = wunder`，避免递归调用；`wunder@doc2md` 提供文档转 Markdown 能力。
...
- 系统提示词会引导模型“先读 SKILL.md 再执行”，两种模式均生效。
...
- 注入与执行：`/wunder/tools` 会把自建工具合入工具清单，`tool_call` 模式下只有被选择的 `tool_names` 才注入提示词；`function_call` 模式仅通过 tools 协议传入。执行阶段通过别名映射找到具体资源，仍受 allow_paths/deny_globs 的安全策略约束。
...
- 共享呈现方式：`/wunder/tools` 将共享工具单独列出并附带 `owner_id`，名称仍为 `owner_id@tool`；默认不注入提示词（`tool_call` 模式），需由用户显式勾选。
...
系统管理工具的核心目标是：统一配置入口、统一工具清单、统一提示词注入（`tool_call` 模式），并在执行阶段严格隔离与可控复用。

```mermaid
flowchart LR
  Config["配置入口(config/wunder.yaml + data/config/wunder.override.yaml + data/user_tools/<user_id>/config.json)"] --> Catalog["工具清单(/wunder/tools, src/api/core.rs + src/services/user_tools.rs)"]
  Catalog --> Orchestrator["编排与注入(src/orchestrator + src/services/prompting.rs)"]
  Orchestrator --> Exec["执行器(src/services/tools.rs + src/services/skills.rs + src/services/knowledge.rs + src/services/mcp.rs)"]
...
- 系统统一汇总工具清单到 `/wunder/tools`，`tool_call` 模式下再根据 `tool_names` 选择性注入提示词，避免不必要的上下文膨胀；`function_call` 模式仅通过 tools 协议传入。
...
#### 5.2.3 事件与监控
- 事件类型：`progress/llm_request/knowledge_request/llm_output_delta/llm_stream_retry/llm_output/quota_usage/tool_call/tool_output_delta/tool_result/plan_update/question_panel/a2ui/token_usage/context_usage/final/error`（observability.monitor_drop_event_types 可过滤落库存储的事件。）
- `final` 事件携带 `stop_reason`（`model_response/final_tool/a2ui/question_panel/max_rounds/unknown`），用于说明停止原因。
- `quota_usage` 事件记录本次模型调用的额度消耗与剩余额度，供用户侧统计展示。
- `context_usage` 事件记录上下文占用量，用于监控与历史回放。
...
- 流式输出会按间隔合并 `llm_output_delta` 写入 `stream_events`，支持刷新后按 event_id 续传并继续推送。

### 5.3 吞吐量测试
- 管理端按并发列表逐档压测，使用内置题库随机发送问题。
- 每个并发档位采样请求统计与总/单预填充、总/单解码速度等指标，前端以性能曲线展示相对增量。
...
## 6. 工作区与数据持久化
- 工作区：`workspaces/<user_id>`（物理存储），系统提示词展示为 `/workspaces/<user_id>/`。
...
## 7. 安全与效率设计
- 接口鉴权：管理员接口使用 `security.api_key`（`X-API-Key`/`Authorization`），用户侧接口使用 `/wunder/auth` 颁发的 Bearer Token。
- 默认管理员账号为 admin/admin，服务启动时自动创建且不可删除，可在用户管理中重置密码。
...
- 监控与资源指标缓存：系统快照/日志占用/工作区占用按 TTL 缓存，日志占用统计数据库日志表，减少频繁数据库查询与磁盘扫描。
...
## 8. 运维与管理入口
- 用户侧前端（Vue3）：默认 18001 端口，目录 `frontend/`，通过 `/wunder/auth`、`/wunder/chat`、`/wunder/workspace` 与 `/wunder/user_tools` 调用后端（附件解析走 `/wunder/chat/attachments/convert`）。
- `/`：管理端调试前端入口（`web/index.html`），包含幻灯片（系统介绍）与 A2A 服务管理面板；`web/simple-chat` 简易聊天测试页暂时停用。
...
- `/wunder/doc2md/convert`：文档转 Markdown（无鉴权，multipart 上传，支持 doc/docx/pdf/pptx/xlsx/ods/odt/odp/wps/dps/et 与常见文本/代码类型，默认上限 200MB）。
- `/wunder/attachments/convert`：管理端调试前端附件解析（需鉴权，解析逻辑与 doc2md 一致）。
- `/wunder/temp_dir/*`：临时文件上传/下载/列出/清理（无鉴权，根目录 `temp_dir/`）。
...
### 8.1 管理端前端页面与接口
- 内部状态：`/wunder/admin/monitor`、`/wunder/admin/monitor/tool_usage`、`/wunder/admin/monitor/{session_id}`、`/wunder/admin/monitor/{session_id}/cancel`。
- 线程管理：`/wunder/admin/users`、`/wunder/admin/users/{user_id}/sessions`、`/wunder/admin/users/{user_id}`（删除线程/工作区数据）。
...
- 模型配置：`/wunder/admin/llm`、`/wunder/admin/llm/context_window`。
- 系统设置：`/wunder/admin/server`、`/wunder/i18n`。
- 内置工具：`/wunder/admin/tools`。
- MCP 服务：`/wunder/admin/mcp`、`/wunder/admin/mcp/tools`、`/wunder/admin/mcp/tools/call`。
- A2A 服务：`/wunder/admin/a2a`、`/wunder/admin/a2a/card`。
- 技能管理：`/wunder/admin/skills`、`/wunder/admin/skills/upload`。
- 知识库管理：`/wunder/admin/knowledge`、`/wunder/admin/knowledge/files`、`/wunder/admin/knowledge/file`、`/wunder/admin/knowledge/upload`、`/wunder/admin/knowledge/refresh`。
- 吞吐量/性能/评估：`/wunder/admin/throughput/*`、`/wunder/admin/performance/sample`、`/wunder/admin/evaluation/*`。
- 调试面板：`/wunder`、`/wunder/system_prompt`、`/wunder/tools`、`/wunder/attachments/convert`、`/wunder/workspace/*`、`/wunder/user_tools/*`。
...
knowledge/           # 公共知识库
temp_dir/            # 临时文件目录（/wunder/temp_dir/*）
EVA_SKILLS/          # 技能包目录
skills/              # 内置技能目录
web/                 # 调试前端（样式入口 app.css，聚合 web/styles/*.css）
web/styles/          # 管理端样式拆分
web/simple-chat/     # 简易聊天测试页（暂时停用）
