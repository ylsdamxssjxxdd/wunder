...
- docker compose（x86/arm）默认可直接 `up`：`wunder-server`、`wunder-sandbox`、`wunder-mcp` 复用同名本地镜像（已存在则优先复用，不存在再自动构建），并避免远端拉取同名镜像。
- docker compose 默认使用两个命名卷：`wunder_workspaces`（`/workspaces`，包含用户工作区 + `/wunder/temp_dir/*` 临时目录，`WUNDER_TEMP_DIR_ROOT` 默认 `/workspaces/temp_dir`）与 `wunder_logs`（PostgreSQL/Weaviate 数据目录）；其余运行态可写配置保留在仓库本地 `data/`。构建/依赖缓存（`target/`、`.cargo/`、`frontend/node_modules/`）保持写入仓库目录便于管理。
...
- `wunder-desktop`：采用“**Tauri 桌面窗口 + 内置本地桥接服务**”模式，默认 `127.0.0.1:27653`（支持 `--port 0`）。
