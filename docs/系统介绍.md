...
- 智能体应用设置页与系统设置页支持权限级别（`approval_mode`）下拉管理；当命令执行/PTC 触发审批时，前端会弹出审批对话框并通过 WebSocket 回传决策。
...
- Tauri 安装包会携带 `config/prompts/skills/scripts/frontend/dist` 资源；启动时优先解析可执行文件同级 `resources`，并将内置 `skills/` 增量同步到本地 `WUNDER_WORK/skills`，确保离线安装后技能工具可用。
