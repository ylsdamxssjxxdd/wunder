...
- 智能体应用设置页与系统设置页支持权限级别（`approval_mode`）下拉管理；当命令执行/PTC 触发审批时，前端会将输入区切换为审批区并通过 WebSocket 回传决策。
...
- Desktop 设置页支持 `tool_call/function_call` 切换与 MCP/Skills 管理；模型配置会持久化到 `desktop.settings.json` 并即时生效；默认轻量运行（关闭 channels/gateway/agent_queue，保留 cron 调度）。
