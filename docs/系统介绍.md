...
- CLI 交互支持 codex 风格 slash 命令：`/help`、`/status`、`/model`、`/tool-call-mode`（`/mode`）与 `/config`。
- `/config` 在 TUI 支持单行参数配置（`/config <base_url> <api_key> <model>`），`chat` 行式模式保留交互引导式配模。
- CLI 提示词改为优先使用二进制内嵌模板，脱离仓库目录也能稳定运行；外部文件仍可选覆盖。
- 流式恢复偏移在空会话下安全回落为 `0`，模型不可用错误会附带可读 detail。

## 运行形态补充：wunder-desktop（2026-02）

- `wunder-desktop` 面向本地普通用户，核心能力与 `wunder-server/wunder-cli` 共用同一套 Rust 核心编排。
- 运行时采用本地桥接服务模式：默认 `127.0.0.1` 随机端口，前端通过 `/wunder` 与 `/wunder/chat/ws` 交互。
