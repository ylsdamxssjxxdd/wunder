...
- 蜂群协作能力。基于 `hive_id` / 蜂巢（`beeroom`）分组隔离智能体，任意带 `agent_swarm` 工具的智能体会话都可触发组内协作，用户侧通过蜂巢工作台统一观测任务闭环。
...
3. 聊天页通过 `SwarmPanel` 回放当前会话的蜂群执行链路；用户也可进入蜂巢（`/beeroom`）工作台，从组维度观察成员、任务和闭环状态。
4. `agent_swarm`、TeamRun 与蜂巢接口统一保留 `hive_id` 作用域参数；若当前会话绑定了蜂巢成员，则默认取该成员所属蜂巢。
5. 首个实际触发蜂群工具的组内智能体会被认领为母蜂；后续任务拆分、子任务派发、结果汇总都以该母蜂为协同锚点。
6. 任务在“所有子任务进入终态 + 所有涉及组员恢复空闲”后视为闭环完成，蜂巢页会将状态标记为 `completed`，否则可能停留在 `running/awaiting_idle/failed/cancelled`。
...
- 若同一轮连续出现同类工具失败，调度器会触发 `tool_failure_guard` 止损并返回可恢复提示，确保线程可继续使用。
