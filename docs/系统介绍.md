...
- 用户世界通信。新增用户↔用户实时通信域（联系人/会话/消息/已读），与智能体会话链路并行隔离。
...
1. 用户侧通过 `/wunder/user_world/contacts` 与 `/conversations` 拉取联系人与会话。
2. 发送消息落库到 `user_world_messages`，并更新会话摘要、成员未读计数与 `user_world_events`。
