...
- Tauri 安装包会携带 `config/prompts/skills/scripts/frontend/dist` 资源；启动时优先解析可执行文件同级 `resources`，并将内置 `skills/` 增量同步到本地 `WUNDER_WORK/skills` 与 `WUNDER_TEMPD/user_tools/<user>/skills`，确保离线安装后技能工具可用。
