...
- 吞吐量测试 + 能力评估：并发压测量化 QPS/延迟与资源占用，用例集评分支持回归对比。
...
## 5. 智能体管理测试功能
### 5.1 总览
- 覆盖线程管理、吞吐量测试与能力评估，用于保障并发稳定并持续量化性能与质量。
- 管理端通过 `/wunder/admin/throughput/*` 与 `/wunder/admin/evaluation/*` 提供压测与评估入口，配合监控面板统一观察。
...
### 5.3 吞吐量测试
- 管理端可触发多用户并发压测，支持多问题随机发送与持续时长控制。
- 统计 QPS、成功率、延迟分桶与 Token usage，并按周期采样系统状态。
