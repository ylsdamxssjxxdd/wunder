...
当 `sandbox.mode=sandbox` 时，内置工具中的“执行命令/ptc”会通过共享沙盒服务执行（`WUNDER_SANDBOX_ENDPOINT`），需要确保沙盒服务已启动并可访问。
...
- `/wunder/web`：调试前端，包含系统介绍页面与 A2A 服务管理面板；系统介绍通过点击侧边栏标题进入，侧边栏支持滚动。
