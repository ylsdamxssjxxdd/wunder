...
- Desktop 设置页支持 `tool_call/function_call/freeform_call` 切换与 MCP/Skills 管理；模型配置会持久化到 `desktop.settings.json` 并即时生效；默认轻量运行（关闭 channels/gateway/agent_queue，保留 cron 调度）。
