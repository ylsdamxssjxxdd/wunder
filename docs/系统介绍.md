...
- 能力评估：用例集评估工具调用/逻辑/常识/复杂任务，结果持久化并支持历史回溯与删除。
...
- 监控层（`src/monitor.rs`）：线程状态与系统资源快照（进程内存）。
- 吞吐量压测（`src/throughput.rs`）：服务端模拟多用户并发请求，支持多问题随机发送，结果可保存/导出并支持历史回放。
- 能力评估（`src/evaluation_runner.rs` + `src/api/evaluation.rs`）：按用例集跑评估，启动时清理该用户 eval 临时目录并预生成明细为 `active`，提供评分与历史回溯/删除。
- 配置层（`src/config.rs` + `src/config_store.rs`）：基础配置与管理端覆盖写入，支持通过管理端调整 `server.max_active_sessions` 并持久化。
...
- 并发互斥当前使用进程内锁实现，后续可接入数据库 `session_locks` 支持跨进程互斥。
...
- 监控数据落库数据库，可在 `/wunder/admin/monitor` 查询实时/历史线程。
...
- Rust 原型阶段监控与记忆使用进程内存，数据库持久化能力后续迁移补齐。
