...
### 2.2 核心模块职责
- API 层（`app/api`）：统一入口 `/wunder`、A2A 标准接口 `/a2a`、工具/技能/知识库管理接口、工作区管理与监控接口。
- 服务层（`app/services`）：配置更新、MCP/A2A 探测、工具清单构建等复用逻辑，避免路由层重复实现。
...
- 存储层（`app/storage` + `app/memory`）：默认 PostgreSQL、可选 SQLite 持久化对话/工具/监控日志与长期记忆，工作区文件持久化。
...
- 并发互斥基于数据库 `session_locks` 表，支持单机多进程的互斥与过期回收，并由心跳续租。
