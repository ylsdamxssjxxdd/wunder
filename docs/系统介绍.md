...
- A2A 服务工具由 `/wunder/admin/a2a` 配置并缓存 AgentCard，启用后以 `a2a@service` 形式注入提示词，用于委派外部 A2A 智能体任务。
- 允许列表：
...
- `/wunder/web`：调试前端，包含系统介绍页面、A2A 服务管理与 A2A 测试面板；系统介绍通过点击侧边栏标题进入，侧边栏支持滚动。
