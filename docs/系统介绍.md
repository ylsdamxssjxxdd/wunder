...
- 能力评估：用例集评估工具调用/逻辑/常识/复杂任务，结果持久化并支持历史对比。
...
- 能力评估（`src/evaluation_runner.rs` + `src/api/evaluation.rs`）：按用例集跑评估，提供评分与历史对比。
- 配置层（`src/config.rs` + `src/config_store.rs`）：基础配置与管理端覆盖写入。
...
- 能力评估结果写入 `evaluation_runs` / `evaluation_items`，用于历史对比与回溯。
