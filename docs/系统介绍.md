...
### 5.3 事件与监控
- 事件类型：`progress/llm_request/knowledge_request/llm_output_delta/llm_stream_retry/llm_output/tool_call/tool_result/token_usage/final/error`（持久化可按 `observability.monitor_drop_event_types` 过滤）。
...
- `/wunder/ppt`：系统介绍 PPT 静态资源（`docs/ppt`），供嵌入与全屏展示。
