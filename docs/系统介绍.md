...
注册用户按访问级别分配每日请求额度，0 点重置；虚拟 user_id 不受额度限制。
...
#### 5.2.3 事件与监控
- 事件类型：`progress/llm_request/knowledge_request/llm_output_delta/llm_stream_retry/llm_output/tool_call/tool_output_delta/tool_result/a2ui/token_usage/final/error`（持久化可按 `observability.monitor_drop_event_types` 过滤）。
