...
- 企业微信回调与直连出站。支持 `/wunder/channel/wechat/webhook`（GET URL 验证 + POST 消息回调），并可用企业微信 API 直接回包。
...
- `wunder-cli`：面向单用户本地使用，持久化目录为 `WUNDER_TEMP/`，工作目录为启动目录；支持审批模式（`approval_mode`）与 codex 风格 TUI（`/approvals`、`/diff`、`/review`、`/mention`）。
