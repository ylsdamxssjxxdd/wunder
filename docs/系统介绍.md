...
- 吞吐量测试 + 性能采样 + 能力评估：并发步增压测量化总/单预填充与总/单解码速度，性能采样观察系统能力基线，用例集评分支持回归对比。
...
- 吞吐量压测（`src/throughput.rs`）：服务端从并发 1 按步增压测到最大并发（步增为 0 时仅压测最大并发），采样总/单预填充与总/单解码速度等指标，支持多问题随机发送，结果可保存/导出并支持历史回放。
