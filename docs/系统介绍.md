...
- 工具调用轮次会将上一轮 assistant 原始输出（`tool_call` 模式含 `<tool_call>` 标签）与 reasoning 追加进下一轮模型请求；`function_call` 模式则回填 assistant 的 `tool_calls` 与 role=tool/tool_call_id 结果，保证上下文可追溯。
