...
- 核心理念是对开发者来说一切都是接口，对大模型来说一切皆工具，对用户来说一切都是智能体线程
...
- 不要尝试创建git分支或提交，这些交给用户，你git diff时可能会遇到出现了不是你修改的内容，没关系那是用户自己改的不用管他，你自行继续就可以了
...
- 区分子智能体工具与蜂群工具：子智能体是主智能体临时创建的新工作单元，默认不阻塞；蜂群工具调用已存在的智能体协作，默认阻塞并汇总
...
- slint能使用官方标准控件的尽量使用，一定使用skills\slint 技能提高开发效率
- 优先使用slint-viewer检验前端效果，而不是构建好程序截图，这样的方法很慢没效率
...
- `builders/`：桌面 Slint 发布、AppImage 打包、Win7 PE 门禁和 Linux/Win32 交叉编译入口；不要把新的构建脚本放回 `frontend-slint/scripts/`。
