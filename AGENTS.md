...
- `frontend\src\stores\chat.ts`、`src\services\tools.rs` 这些文件太大了，请不要再往里面添加新功能了；允许少量接线型改动（如导入新模块、转发调用、注册处理器、补充 selector），但新增功能的主体逻辑、状态结构、事件处理、工具实现应优先放到新的模块文件中；如果你的改动能顺手迁出其中部分代码，就顺手迁出，持续给这些维护文件减负
