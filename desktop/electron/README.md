...
桌面相关构建产物统一落到仓库根 `target/`；`desktop/electron/` 保留 Electron 壳，Tauri 源码位于 `crates/wunder-desktop/`。
...
图标现在采用单一源文件（优先）：`images/eva01-head.png`（若缺失再按环境变量指定源回退）。  
...
- `crates/wunder-desktop/icons/icon.ico`（供 Tauri 打包复用）

`sync-icons.js` 会自动从源图生成多尺寸 `ico`，并对小尺寸图标加入透明边距（可通过 `WUNDER_ICON_PADDING_RATIO` 调整，默认 `0.06`），避免 Windows 桌面快捷方式出现细边框观感。
