#!/usr/bin/env bash
# Cross-build the Win7-compatible wunder CLI executable from an ARM64 Linux
# host. i686-win7-windows-gnu is a Tier 3 target without prebuilt std, so std
# is rebuilt with -Z build-std; the kylin-arm portable MinGW SDK supplies
# linker, CRT and import libraries.
set -euo pipefail
...
target_dir="${CARGO_TARGET_DIR:-$repo_root/target/cli-win7-x86-arm64/cargo}"
cargo_home="${CARGO_HOME:-$repo_root/target/cli-win7-x86-arm64/cargo-home}"
output_dir="${WUNDER_CLI_OUTPUT_DIR:-$repo_root/target/cli/dist/win7-x86}"

fail() { echo "[wunder-cli-win7-arm64] $*" >&2; exit 2; }
...
echo "[wunder-cli-win7-arm64] cargo: $(cargo --version)"
cd "$repo_root"
cargo -Z build-std=std,panic_abort build --locked --offline --release \
  -p wunder-cli --bin wunder-cli --target "$target" -j "${WUNDER_BUILD_JOBS:-2}"
...
release="$output_dir/wunder-cli-$version-win7-x86.exe"
cp -f "$exe" "$release"
printf '%s\n' "$imports" > "$output_dir/imports.txt"
echo "[wunder-cli-win7-arm64] Win7 PE import gate passed."
echo "[wunder-cli-win7-arm64] produced: $release"
