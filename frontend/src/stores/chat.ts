...
const resolveExplicitContextTotalTokens = (source) => {
  if (!source || typeof source !== 'object') return null;
  const record = source as Record<string, unknown>;
  const contextUsage = resolveContextUsageRecord(record);
...
const sessionSubagentsCache = new Map<string, { cachedAt: number; items: unknown[] }>();
...
const SESSION_SUBAGENTS_CACHE_TTL_MS = 12 * 1000;
  ...
  const handleEvent = (eventName, raw) => {
    ...
    // 基于事件类型生成工作流条目并更新回复内容
      ...
      case 'final': {
        ...
        updateRoundUsageStats(finalPayload ?? data ?? payload ?? {});
        updateBackendTurnSpeedStats(finalPayload);
        outputState.streaming = false;
        outputState.reasoningStreaming = false;
        syncReasoningToMessage();
