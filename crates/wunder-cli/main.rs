...
use args::{
    ApprovalModeArg, AskCommand, Cli, Command, CompletionCommand, ConfigCommand, ConfigSubcommand,
...
fn dispatch_command<'a>(
    ...
    match command {
        Command::Ask(cmd) => Box::pin(handle_ask(runtime, global, cmd)),
        Command::Chat(cmd) => Box::pin(run_chat_loop(runtime, global, cmd.prompt, None)),
