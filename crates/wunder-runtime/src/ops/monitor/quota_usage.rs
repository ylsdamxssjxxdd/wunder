...
impl MonitorState {
    /// Count successful quota admissions, including retries and hidden summary calls.
    /// An old record without a baseline remains unknown rather than reporting a partial total.
    pub fn record_quota_consumption(&self, session_id: &str, consumed: i64) -> Option<i64> {
        if consumed <= 0 {
            return None;
        }
        let now = now_ts();
        let (total, to_persist) = {
            ...
            record.quota_used = record
                .quota_used
                .map(|value| value.saturating_add(consumed));
...
mod tests {
    ...
    fn thread_quota_survives_event_eviction_storage_reload_and_new_turns() {
        ...
        assert_eq!(monitor.record_quota_consumption("session_1", 1), Some(1));
        assert_eq!(monitor.record_quota_consumption("session_1", 1), Some(2));
        monitor.register("session_1", "user_1", "", "", false, false);
        assert_eq!(monitor.record_quota_consumption("session_1", 1), Some(3));
        monitor.record_event(
            "session_1",
            "quota_usage",
            &json!({"session_quota_used":3,"consumed":1}),
        );
