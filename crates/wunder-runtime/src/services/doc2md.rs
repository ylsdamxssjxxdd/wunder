...
#[cfg(feature = "doc2md")]
use rlsx::{parse_xlsx_to_workbook, CellValue as RlsxCellValue};
...
#[cfg_attr(not(feature = "doc2md"), allow(dead_code))]
#[derive(Debug, Clone)]
...
#[cfg(not(feature = "doc2md"))]
const HEAVY_EXTENSIONS: &[&str] = &[".doc", ".dps", ".et", ".ods", ".pdf", ".wps", ".xlsx"];

pub fn supported_extensions() -> Vec<String> {
    let mut exts = SUPPORTED_EXTENSIONS
        .iter()
        .filter(|ext| doc2md_extension_enabled(ext))
...
#[cfg(feature = "doc2md")]
fn doc2md_extension_enabled(_extension: &&str) -> bool {
    true
}

#[cfg(not(feature = "doc2md"))]
fn doc2md_extension_enabled(extension: &&str) -> bool {
    !HEAVY_EXTENSIONS.contains(extension)
}
...
fn convert_sync(path: &Path, extension: &str) -> Result<Doc2mdResult> {
    ...
    require_heavy_doc2md_feature(&ext)?;
...
#[cfg(feature = "doc2md")]
fn require_heavy_doc2md_feature(_extension: &str) -> Result<()> {
    Ok(())
}

#[cfg(not(feature = "doc2md"))]
fn require_heavy_doc2md_feature(extension: &str) -> Result<()> {
    if HEAVY_EXTENSIONS.contains(&extension) {
        return Err(anyhow!(
            "doc2md heavy document parsing is disabled; rebuild with the doc2md feature"
        ));
    }
    Ok(())
}
...
fn convert_pptx(path: &Path) -> Result<Doc2mdResult> {
    ...
    for (index, xml) in slides {
        let mut paragraphs = parse_pptx_xml(&xml).unwrap_or_default();
        if paragraphs.is_empty() {
            let fallback = parse_pptx_xml_fallback(&xml);
            if !fallback.is_empty() {
                paragraphs = fallback;
                warnings.push(format!("pptx fallback parser used for slide {index}"));
            }
        }
        if paragraphs.is_empty() {
            continue;
...
fn convert_pdf(path: &Path) -> Result<Doc2mdResult> {
    #[cfg(not(feature = "doc2md"))]
    {
        let _ = path;
        return Err(anyhow!(
            "pdf parsing requires rebuilding with the doc2md feature"
        ));
    }
    ...
    {
        let text =
            extract_pdf_text(path).map_err(|err| anyhow!(format!("pdf parse failed: {err}")))?;
...
fn convert_doc_binary(path: &Path) -> Result<Doc2mdResult> {
    #[cfg(not(feature = "doc2md"))]
    {
        let _ = path;
        return Err(anyhow!(
            "legacy office parsing requires rebuilding with the doc2md feature"
        ));
    }
...
fn convert_et(path: &Path) -> Result<Doc2mdResult> {
    #[cfg(not(feature = "doc2md"))]
    {
        let _ = path;
        return Err(anyhow!(
            "legacy spreadsheet parsing requires rebuilding with the doc2md feature"
        ));
    }
...
fn convert_spreadsheet(path: &Path) -> Result<Doc2mdResult> {
    #[cfg(not(feature = "doc2md"))]
    {
        let _ = path;
        return Err(anyhow!(
            "spreadsheet parsing requires rebuilding with the doc2md feature"
        ));
    }
    ...
    #[cfg(feature = "doc2md")]
    {
        if detect_zip_kind(path) == Some("xlsx") {
            match convert_xlsx_rlsx(path) {
                Ok(result) => return Ok(result),
                Err(err) => {
                    if let Ok(mut result) = convert_spreadsheet_calamine(path) {
                        result.warnings.push(format!("rlsx failed: {err}"));
                        return Ok(result);
                    }
                    return Err(anyhow!(format!("spreadsheet open failed: {err}")));
                }
            }
        }
        let mut workbook = convert_spreadsheet_calamine_workbook(path)?;
...
#[cfg(feature = "doc2md")]
fn convert_spreadsheet_calamine(path: &Path) -> Result<Doc2mdResult> {
    let mut workbook = convert_spreadsheet_calamine_workbook(path)?;
    let sheet_names = workbook.sheet_names().to_owned();
    let mut blocks = Vec::new();
    for name in sheet_names {
...
#[cfg(feature = "doc2md")]
fn convert_spreadsheet_calamine_workbook(
    path: &Path,
) -> Result<calamine::Sheets<std::io::BufReader<std::fs::File>>> {
    open_workbook_auto(path).map_err(|err| anyhow!(format!("spreadsheet open failed: {err}")))
}

#[cfg(feature = "doc2md")]
fn convert_xlsx_rlsx(path: &Path) -> Result<Doc2mdResult> {
    let workbook =
        parse_xlsx_to_workbook(path).map_err(|err| anyhow!(format!("xlsx parse failed: {err}")))?;
    let mut blocks = Vec::new();
    for (index, sheet) in workbook.sheets.iter().enumerate() {
        let rows = rlsx_sheet_rows_to_matrix(&sheet.rows);
...
#[cfg(feature = "doc2md")]
fn rlsx_sheet_rows_to_matrix(rows: &[Vec<rlsx::Cell>]) -> Vec<Vec<String>> {
    ...
    for row in rows {
        ...
        for cell in row {
            if let Some((_, col)) = parse_cell_ref(&cell.address) {
                let value = rlsx_cell_value_to_string(&cell.value);
        ...
        let mut row_values = vec![String::new(); max_col + 1];
...
#[cfg(feature = "doc2md")]
fn rlsx_cell_value_to_string(value: &RlsxCellValue) -> String {
    match value {
        RlsxCellValue::String(text) => text.clone(),
        RlsxCellValue::Number(num) => num.to_string(),
        RlsxCellValue::Bool(flag) => {
            if *flag {
                "TRUE".to_string()
            } else {
                "FALSE".to_string()
        ...
        RlsxCellValue::Date(dt) => dt.to_rfc3339(),
        RlsxCellValue::Empty => String::new(),
    }
}
...
fn parse_pptx_xml_fallback(xml: &str) -> Vec<String> {
    ...
    for caps in para_re.captures_iter(xml) {
        ...
        for token in token_re.captures_iter(block) {
            if let Some(raw) = token.name("text") {
                text.push_str(&decode_xml_entities(raw.as_str()));
            } else if token.name("br").is_some() {
                text.push('\n');
            } else if token.name("tab").is_some() {
                text.push(' ');
            }
        }
        ...
        if let Ok(mut ole) = cfb::CompoundFile::open(file) {
            ...
            for entry in ole.walk() {
                ...
                if entry_name.eq_ignore_ascii_case(name) {
                    fallback_path = Some(entry.path().to_path_buf());
                    break;
                }
                if fallback_path.is_none() && entry_name.to_ascii_lowercase().contains(&target) {
                    fallback_path = Some(entry.path().to_path_buf());
                }
            }
            if let Some(path) = fallback_path {
                if let Ok(mut stream) = ole.open_stream(path) {
                    let mut data = Vec::new();
                    if stream.read_to_end(&mut data).is_ok() {
                        return Some(data);
                    }
...
mod tests {
    ...
    #[test]
    fn supported_extensions_match_doc2md_feature_boundary() {
