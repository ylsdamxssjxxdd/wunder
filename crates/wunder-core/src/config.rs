...
use crate::drawio_config::DrawioConfig;
use crate::onlyoffice_config::OnlyOfficeConfig;
...
pub fn load_config_from_path(path: &Path) -> Config {
    ...
    let mut config = serde_yaml::from_value::<Config>(value).unwrap_or_else(|err| {
        eprintln!("failed to parse config, falling back to defaults: {err}");
        Config::default()
    });
...
fn read_yaml_path(path: &Path) -> Value {
    ...
    let content = match read_yaml_content_with_fallback(&path_display) {
        Ok(text) => text,
        Err(err) => {
            eprintln!("failed to read config file: {path_display}, {err}");
            return Value::Null;
        }
    };
    serde_yaml::from_str(&content).unwrap_or_else(|err| {
        eprintln!("failed to parse YAML: {path_display}, {err}");
        Value::Null
    })
}
...
"#,
    ...
    fn test_load_config_from_path_repairs_generated_sparse_config_from_example() {
        let root =
            std::env::temp_dir().join(format!("wunder-config-repair-{}", unique_test_suffix()));
        fs::create_dir_all(&root).expect("create temp dir");
        let config_path = root.join("wunder.yaml");
...
"#,
    ...
    fn test_read_yaml_content_falls_back_to_yml_variant() {
        let root = std::env::temp_dir().join(format!("wunder-config-yml-{}", unique_test_suffix()));
        fs::create_dir_all(&root).expect("create temp dir");
    ...
    fn test_read_yaml_tolerates_utf8_bom() {
        let root = std::env::temp_dir().join(format!("wunder-config-bom-{}", unique_test_suffix()));
        fs::create_dir_all(&root).expect("create temp dir");
        let yaml_path = root.join("wunder.yaml");
