...
- 安装包：`target/release/bundle/nsis/wunder-desktop_0.3.0_x64-setup.exe`
- Tauri/Cargo 产物统一写入仓库根 `target/`
...
```powershell
powershell -ExecutionPolicy Bypass -File crates/wunder-desktop/scripts/build-win7.ps1
```

- 默认使用 `temp_dir/win7-lab/cargo-home` 与 `temp_dir/win7-lab/tauri-build-target`
- 默认优先使用 `crates/wunder-desktop/webview2/win7-x86/` 下的 Fixed Runtime 109；如果目录不存在，则自动退回 `skip` 模式，仅验证打包链是否可产出 NSIS 包
...
```powershell
# 推荐先试 x64 GNU
powershell -ExecutionPolicy Bypass -File crates/wunder-desktop/scripts/build-win7-gnu.ps1 -Arch x64

# 若继续试 32 位 GNU
powershell -ExecutionPolicy Bypass -File crates/wunder-desktop/scripts/build-win7-gnu.ps1 -Arch ia32
```
...
- JavaScript 依赖统一安装在仓库根目录 `node_modules/`
  - `crates/wunder-desktop` 本身不维护单独的 `node_modules/`。
