...
#[cfg(feature = "desktop")]
fn icons_need_sync(repo_root: &Path, icon_source_path: &Path) -> bool {
    let Some(source_mtime) = read_modified(icon_source_path) else {
        return false;
    };
    let targets = [
        repo_root.join("desktop/electron/build/icon.png"),
        repo_root.join("desktop/electron/build/icon.ico"),
        repo_root.join("desktop/electron/assets/icon.ico"),
        repo_root.join("crates/wunder-desktop/icons/icon.ico"),
    ];
...
fn resolve_repo_root_and_tauri_dir(manifest_dir: &Path) -> (PathBuf, PathBuf) {
    let root_layout_tauri_dir = manifest_dir.join("crates/wunder-desktop");
